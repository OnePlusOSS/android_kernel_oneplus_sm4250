...
/* oneplus_kevent_upload.c - for kevent action upload,root action upload to user layer
...
int kevent_send_to_user(struct kernel_packet_info *userinfo)
{
	...
	if (ret < 0) {
		pr_info("s%:netlink_unicast: can not unicast skbuff\n", __func__);
		return -EFAULT;
	}
...
void kernel_kevent_receive(struct sk_buff *__skbbr)
{
	...
	if (skbu->len >= sizeof(struct nlmsghdr)) {
		...
		if ((nlh->nlmsg_len >= sizeof(struct nlmsghdr)) && (__skbbr->len >= nlh->nlmsg_len))
...
int __init netlink_kevent_init(void)
{
	...
	netlink_fd = netlink_kernel_create(&init_net, NETLINK_ONEPLUS_KEVENT, &cfg);
	if (!netlink_fd) {
