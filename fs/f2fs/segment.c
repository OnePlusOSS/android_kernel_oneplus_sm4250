...
static inline void of2fs_balance_fs(struct f2fs_sb_info *sbi)
{
	...
	if (has_not_enough_free_secs(sbi, 0, 0)) {
		...
		if (gc_th != NULL) {
			...
			f2fs_info(sbi, "Debug:%s:begin wait trigger fggc,gc_opt_enable:%d",
					__func__, sbi->gc_opt_enable);

			prepare_to_wait(&gc_th->fggc_wait_queue_head,
					&__wait, TASK_UNINTERRUPTIBLE);
			f2fs_info(sbi, "Debug:%s:finish wait trigger fggc,gc_opt_enable:%d",
					__func__, sbi->gc_opt_enable);
			wake_up(&gc_th->gc_wait_queue_head);
			schedule();
			finish_wait(&gc_th->fggc_wait_queue_head, &__wait);
		} else {
			f2fs_info(sbi, "Debug:%s:no-gcthread,fggc,gc_opt_enable:%d",
					__func__, sbi->gc_opt_enable);
			mutex_lock(&sbi->gc_mutex);
			f2fs_gc(sbi, false, false, NULL_SEGNO);
		}
		...
		if (gc_th != NULL) {
			f2fs_info(sbi, "Debug:%s:tigger SSR,gc_opt_enable:%d",
					__func__, sbi->gc_opt_enable);
...
void f2fs_balance_fs(struct f2fs_sb_info *sbi, bool need)
{
	...
	if (has_not_enough_free_secs(sbi, 0, 0)) {
		mutex_lock(&sbi->gc_mutex);
		f2fs_info(sbi, "Debug:%s:original fggc", __func__);
		f2fs_gc(sbi, false, false, NULL_SEGNO);
	}
#endif
}
...
static int issue_discard_thread(void *data)
{
	...
	do {
		...
		if (f2fs_odiscard_enable) {
			...
			if (f2fs_readonly(sbi->sb))
				continue;
			if (kthread_should_stop()) {
				f2fs_info(sbi, "Debug:%s:discard thread return f2fs_odiscard_enable:%d",
						__func__, sbi->gc_opt_enable);
				return 0;
			}
			if (is_sbi_flag_set(sbi, SBI_NEED_FSCK)) {
			...
			if (f2fs_readonly(sbi->sb))
				continue;
			if (kthread_should_stop()) {
				f2fs_info(sbi, "Debug:%s:non-jumpedin, discard thread return", __func__);
				return 0;
			}
			if (is_sbi_flag_set(sbi, SBI_NEED_FSCK)) {
				wait_ms = dpolicy.max_interval;
	...
	f2fs_info(sbi, "Debug:%s:discard return", __func__);
	return 0;
}
