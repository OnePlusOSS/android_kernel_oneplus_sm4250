...
static inline bool of2fs_gc_wait(struct f2fs_sb_info *sbi, wait_queue_head_t *wq, unsigned int *wait_ms)
{
	...
	f2fs_info(sbi, "Debug:%s:gc_opt_enable:%d", __func__, sbi->gc_opt_enable);
...
static int gc_thread_func(void *data)
{
	...
	do {
		...
		if (try_to_freeze()) {
			...
			f2fs_info(sbi, "Debug:%s:skip BG_GC cause try_to_freeze,wait:%ums",
							__func__, wait_ms);
			continue;
		}
		...
		if (sbi->sb->s_writers.frozen >= SB_FREEZE_WRITE) {
			...
			stat_other_skip_bggc_count(sbi);
			f2fs_info(sbi, "Debug:%s:skip BG_GC cause FREEZE-writer,wait:%ums",
							__func__, wait_ms);
			continue;
		}
		...
		if (!sb_start_write_trylock(sbi->sb)) {
			stat_other_skip_bggc_count(sbi);
			f2fs_info(sbi, "Debug:%s:skip BG_GC cause sblock,wait:%ums",
							__func__, wait_ms);
			continue;
		}
		...
		if (!mutex_trylock(&sbi->gc_mutex)) {
			stat_other_skip_bggc_count(sbi);
			f2fs_info(sbi, "Debug:%s:skip BG_GC cause gclock,wait:%ums", __func__, wait_ms);
			goto next;
		}
		...
		if (!is_idle(sbi, GC_TIME)) {
			...
			f2fs_info(sbi, "Debug:%s:skip BG_GC cause not idle,wait:%ums", __func__, wait_ms);
			goto next;
		}
...
do_gc:
		...
		f2fs_info(sbi, "Debug:%s:do BG_GC,wait:%ums", __func__, wait_ms);
...
next:
	...
	f2fs_info(sbi, "Debug:%s:gc_opt_return", __func__);
	return 0;
}
...
int f2fs_start_gc_thread(struct f2fs_sb_info *sbi)
{
	...
	gc_th->gc_wake = 0;
	f2fs_info(sbi, "Debug:%s:start gc thread", __func__);
