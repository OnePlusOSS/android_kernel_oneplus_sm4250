...
extern int oneplus_update_aod_light_mode_unlock(struct dsi_panel *panel);
extern int oneplus_display_update_aod_area_unlock(void);
extern int oneplus_set_aod_gamma_data_status(struct dsi_panel *panel);
int oneplus_dfps_idle_off = 0;
...
extern int oneplus_dc2_alpha;
int oneplus_seed_backlight = 0;
static struct dsi_panel_cmd_set oneplus_priv_seed_cmd_set;
extern int oneplus_dimlayer_bl_alpha_value;
extern int oneplus_seed_bright_to_alpha(int brightness);
static struct dsi_panel_cmd_set *
oneplus_dsi_update_seed_backlight(struct dsi_panel *panel, int brightness,
				enum dsi_cmd_set_type type)
{
	enum dsi_cmd_set_state state;
	struct dsi_cmd_desc *cmds;
	struct dsi_cmd_desc *oneplus_cmd;
	u8 *tx_buf;
	int count, rc = 0;
	int i = 0;
	int k = 0;
	int alpha = oneplus_seed_bright_to_alpha(brightness);
	...
	oneplus_cmd = kmemdup(cmds, sizeof(*cmds) * count, GFP_KERNEL);
	if (!oneplus_cmd) {
		rc = -ENOMEM;
		goto error;
	}
	...
	for (i = 0; i < count; i++)
		oneplus_cmd[i].msg.tx_buf = NULL;

	for (i = 0; i < count; i++) {
		u32 size;

		size = oneplus_cmd[i].msg.tx_len * sizeof(u8);

		oneplus_cmd[i].msg.tx_buf = kmemdup(cmds[i].msg.tx_buf, size, GFP_KERNEL);
		if (!oneplus_cmd[i].msg.tx_buf) {
			rc = -ENOMEM;
			goto error;
		}
	}
	...
	for (i = 0; i < count; i++) {
		if (oneplus_cmd[i].msg.tx_len != 0x16)
			continue;
		tx_buf = (u8 *)oneplus_cmd[i].msg.tx_buf;
		for (k = 0; k < oneplus_cmd[i].msg.tx_len; k++) {
			if (k == 0) {
				continue;
			}
	...
	if (oneplus_priv_seed_cmd_set.cmds) {
		for (i = 0; i < oneplus_priv_seed_cmd_set.count; i++)
			kfree(oneplus_priv_seed_cmd_set.cmds[i].msg.tx_buf);
		kfree(oneplus_priv_seed_cmd_set.cmds);
	}

	oneplus_priv_seed_cmd_set.cmds = oneplus_cmd;
	oneplus_priv_seed_cmd_set.count = count;
	oneplus_priv_seed_cmd_set.state = state;
	oneplus_dc2_alpha = alpha;

	return &oneplus_priv_seed_cmd_set;

error:
	if (oneplus_cmd) {
		for (i = 0; i < count; i++)
			kfree(oneplus_cmd[i].msg.tx_buf);
		kfree(oneplus_cmd);
	}
	return ERR_PTR(rc);
}
				...
				enum dsi_cmd_set_type type)
{
	...
	struct dsi_panel_cmd_set *oneplus_cmd_set = NULL;
	...
	if (oneplus_seed_backlight) {
		oneplus_cmd_set = oneplus_dsi_update_seed_backlight(panel, oneplus_seed_backlight, type);
		if (!IS_ERR_OR_NULL(oneplus_cmd_set)) {
			cmds = oneplus_cmd_set->cmds;
			count = oneplus_cmd_set->count;
			state = oneplus_cmd_set->state;
		}
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (panel->oneplus_priv.skip_mipi_last_cmd)
			cmds->msg.flags &= ~MIPI_DSI_MSG_LASTCOMMAND;
...
extern int oneplus_dimlayer_bl_alpha;
extern int oneplus_dimlayer_bl_enabled;
extern int oneplus_dimlayer_bl_enable_real;
ktime_t oneplus_backlight_time;
u32 oneplus_last_backlight = 0;
u32 oneplus_backlight_delta = 0;
extern int oneplus_panel_process_dimming_v2(struct dsi_panel *panel, int bl_lvl, bool force_disable);
extern int oneplus_panel_process_dimming_v3(struct dsi_panel *panel, int brightness);
extern int oneplus_panel_update_seed_mode_unlock(struct dsi_panel *panel);
extern void oneplus_panel_process_dimming_v2_post(struct dsi_panel *panel, bool force_disable);
extern int oneplus_display_mode;
	...
	u32 bl_lvl)
{
	...
	if ((get_oneplus_display_scene() == ONEPLUS_DISPLAY_AOD_SCENE) && ( bl_lvl == 1)) {
		pr_err("dsi_cmd AOD mode return bl_lvl:%d\n",bl_lvl);
		return 0;
	}

	if (panel->is_hbm_enabled || (panel->oneplus_priv.is_aod_ramless && !oneplus_display_mode))
		return rc;

	if (bl_lvl > 1) {
		if (bl_lvl > oneplus_last_backlight)
			oneplus_backlight_delta = bl_lvl - oneplus_last_backlight;
		else
			oneplus_backlight_delta = oneplus_last_backlight - bl_lvl;
		oneplus_backlight_time = ktime_get();
	}
	if (oneplus_dimlayer_bl_enabled != oneplus_dimlayer_bl_enable_real) {
		oneplus_dimlayer_bl_enable_real = oneplus_dimlayer_bl_enabled;
		if (oneplus_dimlayer_bl_enable_real) {
	...
	bl_lvl = oneplus_panel_process_dimming_v2(panel, bl_lvl, false);
	bl_lvl = oneplus_panel_process_dimming_v3(panel, bl_lvl);
	...
	if (!strcmp(panel->name,"samsung ams644va04 amoled fhd+ panel") ||
	    ...
	    !strcmp(panel->name,"samsung ams643xf01 amoled fhd+ panel")) {
	...
	oneplus_panel_process_dimming_v2_post(panel, false);
	oneplus_last_backlight = bl_lvl;
...
static int dsi_panel_parse_oneplus_fod_config(struct dsi_panel *panel)
{
	int rc = 0;
	int i;
	u32 length = 0;
	...
	const u32 *arr;
	struct dsi_parser_utils *utils = &panel->utils;
	struct oneplus_brightness_alpha *seq;

	if (panel->host_config.ext_bridge_mode)
		return 0;

	arr = utils->get_property(utils->data, "oneplus,dsi-fod-brightness", &length);
	if (!arr) {
		DSI_ERR("[%s] oneplus,dsi-fod-brightness  not found\n", panel->name);
		return -EINVAL;
	}

	if (length & 0x1) {
		DSI_ERR("[%s] oneplus,dsi-fod-brightness length error\n", panel->name);
		return -EINVAL;
	}
	...
	rc = utils->read_u32_array(utils->data, "oneplus,dsi-fod-brightness",
					arr_32, length);
	if (rc) {
...
static int dsi_panel_parse_oneplus_backlight_remapping_config(struct dsi_panel *panel)
{
	int rc = 0;
	int i;
	u32 length = 0;
	...
	const u32 *arr;
	struct dsi_parser_utils *utils = &panel->utils;
	struct oneplus_brightness_alpha *bl_remap;

	if (panel->host_config.ext_bridge_mode)
		return 0;

	arr = utils->get_property(utils->data, "oneplus,dsi-brightness-remapping", &length);
	if (!arr) {
		DSI_ERR("[%s] oneplus,dsi-fod-brightness  not found\n", panel->name);
		return -EINVAL;
	}
	...
	rc = utils->read_u32_array(utils->data, "oneplus,dsi-brightness-remapping",
					arr_32, length);
	if (rc) {
		DSI_ERR("[%s] cannot read oneplus,dsi-brightness-remapping\n", panel->name);
		goto error_free_arr_32;
	}
	...
	panel->oneplus_priv.bl_remap = bl_remap;
	panel->oneplus_priv.bl_remap_count = count;
...
#define DSI_PANEL_ONEPLUS_DUMMY_VENDOR_NAME  "PanelVendorDummy"
#define DSI_PANEL_ONEPLUS_DUMMY_MANUFACTURE_NAME  "dummy1024"
static int dsi_panel_parse_oneplus_config(struct dsi_panel *panel)
{
	struct dsi_parser_utils *utils = &panel->utils;

	dsi_panel_parse_oneplus_fod_config(panel);
	dsi_panel_parse_oneplus_backlight_remapping_config(panel);

	panel->oneplus_priv.vendor_name = utils->get_property(utils->data,
				"oneplus,mdss-dsi-vendor-name", NULL);
	if (!panel->oneplus_priv.vendor_name) {
		pr_err("Failed to found panel name, using dumming name\n");
		panel->oneplus_priv.vendor_name = DSI_PANEL_ONEPLUS_DUMMY_VENDOR_NAME;
	}
	panel->oneplus_priv.manufacture_name = utils->get_property(utils->data,
				"oneplus,mdss-dsi-manufacture", NULL);
	if (!panel->oneplus_priv.manufacture_name) {
		pr_err("Failed to found panel name, using dumming name\n");
		panel->oneplus_priv.manufacture_name = DSI_PANEL_ONEPLUS_DUMMY_MANUFACTURE_NAME;
	}
	...
	panel->oneplus_priv.is_aod_ramless = utils->read_bool(utils->data,
		"oneplus,aod_ramless");
	DSI_INFO("aod ramless mode: %s", panel->oneplus_priv.is_aod_ramless ? "true" : "false");

	panel->oneplus_priv.dfps_idle_off = utils->read_bool(utils->data,
		"oneplus,dfps-idle-off");
	DSI_INFO("oneplus,dfps-idle-off: %s", panel->oneplus_priv.dfps_idle_off ? "true" : "false");
	oneplus_dfps_idle_off = panel->oneplus_priv.dfps_idle_off;

	return 0;
}

static int dsi_panel_parse_oneplus_mode_config(struct dsi_display_mode *mode,
				struct dsi_parser_utils *utils)
{
	int rc;
	struct dsi_display_mode_priv_info *priv_info;
	...
	rc = utils->read_u32(utils->data, "oneplus,fod-on-vblank", &val);
	if (rc) {
		DSI_ERR("oneplus,fod-on-vblank is not defined, rc=%d\n", rc);
		priv_info->fod_on_vblank = 0;
	} else {
		priv_info->fod_on_vblank = val;
		DSI_INFO("oneplus,fod-on-vblank is %d", val);
	}

	rc = utils->read_u32(utils->data, "oneplus,fod-off-vblank", &val);
	if (rc) {
		DSI_ERR("oneplus,fod-on-vblank is not defined, rc=%d\n", rc);
		priv_info->fod_off_vblank = 0;
	} else {
		priv_info->fod_off_vblank = val;
		DSI_INFO("oneplus,fod-off-vblank is %d", val);
	}
				...
				int topology_override)
{
	...
	rc = dsi_panel_parse_oneplus_config(panel);
	if (rc)
			...
			int topology_override)
{
	...
	dsi_for_each_child_node(timings_np, child_np) {
		...
		rc = dsi_panel_parse_oneplus_mode_config(mode, utils);
		if (rc)
			DSI_ERR(
			"failed to parse oneplus config, rc=%d\n", rc);
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_LP1 cmd, rc=%d\n",
		       panel->name, rc);
	oneplus_set_aod_gamma_data_status(panel);
	oneplus_update_aod_light_mode_unlock(panel);
	panel->need_power_on_backlight = true;
	set_oneplus_display_power_status(ONEPLUS_DISPLAY_POWER_DOZE);
...
int dsi_panel_set_lp2(struct dsi_panel *panel)
{
	...
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_LP2 cmd, rc=%d\n",
		       panel->name, rc);
	set_oneplus_display_power_status(ONEPLUS_DISPLAY_POWER_DOZE_SUSPEND);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_NOLP cmd, rc=%d\n",
		       panel->name, rc);
	set_oneplus_display_power_status(ONEPLUS_DISPLAY_POWER_ON);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	set_oneplus_display_power_status(ONEPLUS_DISPLAY_POWER_OFF);
	panel->power_mode = SDE_MODE_DPMS_OFF;
