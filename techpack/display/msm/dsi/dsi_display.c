...
extern int oneplus_dsi_update_seed_mode(void);
extern int oneplus_dsi_update_spr_mode(void);
extern int oneplus_dsi_update_dynamic_osc_clock(void);
/* Don't panic if smmu fault*/
extern int sde_kms_set_smmu_no_fatal_faults(struct drm_device *drm);
extern int oneplus_set_display_vendor(struct dsi_display *display);
...
extern int oneplus_start_ffl_thread(void);
extern void oneplus_stop_ffl_thread(void);
extern int oneplus_display_get_panel_production_date_serial_number(void);
		...
		void *display, u32 bl_lvl)
{
	...
	panel = dsi_display->panel;
		...
		if ((bl_lvl > 1) && (readcount == 0) && (get_oneplus_display_power_status() == ONEPLUS_DISPLAY_POWER_ON)) {
			oneplus_display_get_panel_production_date_serial_number();
			pr_err("dsi_cmd oneplus_display_get_panel_production_date_serial_number\n");
		readcount = 1;
	}
	...
	if (panel->need_power_on_backlight && bl_lvl) {
		...
		oneplus_start_ffl_thread();
	}

	panel->bl_config.bl_level = bl_lvl;

	if (oneplus_ffl_trigger_finish == false)
		goto error;
		...
		int power_mode, void *disp)
{
	...
	struct msm_drm_notifier notifier_data_f2fs;
	int blank_f2fs;
#endif
	...
	case SDE_MODE_DPMS_LP2:
		...
		switch(get_oneplus_display_scene()) {
		case ONEPLUS_DISPLAY_NORMAL_SCENE:
		case ONEPLUS_DISPLAY_NORMAL_HBM_SCENE:
			rc = dsi_panel_set_lp1(display->panel);
			rc = dsi_panel_set_lp2(display->panel);
			set_oneplus_display_scene(ONEPLUS_DISPLAY_AOD_SCENE);
			break;
		case ONEPLUS_DISPLAY_AOD_HBM_SCENE:
			...
			if (!sde_connector_get_fppress_mode(connector)) {
				...
				if (display->panel->panel_initialized) {
					...
					oneplus_update_aod_light_mode_unlock(display->panel);
				} else {
			...
			break;
		case ONEPLUS_DISPLAY_AOD_SCENE:
		default:
			break;
		}
		if (notify_off)
...
#ifdef CONFIG_F2FS_OF2FS
		blank_f2fs = DRM_PANEL_BLANK_POWERDOWN;
		notifier_data_f2fs.data = &blank_f2fs;
		f2fs_panel_notifier_call_chain(DRM_PANEL_EARLY_EVENT_BLANK, &notifier_data_f2fs);
		break;
#endif
		...
		void *data)
{
	...
	if(0 != oneplus_set_display_vendor(display)) {
		pr_err("maybe send a null point to oneplus display manager\n");
	}
...
static int dsi_display_init(struct dsi_display *display)
{
	...
	if (rc) {
		...
		scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$component add error panel match fault rc=%d",ONEPLUS_MM_DIRVER_FB_EVENT_ID_PANEL_MATCH_FAULT,rc);
...
extern ktime_t oneplus_onscreenfp_pressed_time;
extern u32 oneplus_onscreenfp_vblank_count;
	...
	if (display->panel->cur_mode->dsi_mode_flags & DSI_MODE_FLAG_POMS) {
		...
		if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
			if (display->panel->oneplus_priv.is_aod_ramless &&
				...
				oneplus_onscreenfp_vblank_count = drm_crtc_vblank_count(display->drm_conn->state->crtc);
