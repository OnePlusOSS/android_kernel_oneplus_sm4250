...
extern int oneplus_debug_max_brightness;
...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if (oneplus_debug_max_brightness) {
		bl_lvl = mult_frac(brightness, oneplus_debug_max_brightness,
			display->panel->bl_config.brightness_max_level);
	} else if (brightness == 0) {
		bl_lvl = 0;
	} else {
		if (display->panel->oneplus_priv.bl_remap && display->panel->oneplus_priv.bl_remap_count) {
			int i = 0;
			int count = display->panel->oneplus_priv.bl_remap_count;
			struct oneplus_brightness_alpha *lut = display->panel->oneplus_priv.bl_remap;

			for (i = 0; i < display->panel->oneplus_priv.bl_remap_count; i++){
				if (display->panel->oneplus_priv.bl_remap[i].brightness >= brightness)
					break;
			}
...
extern int oneplus_display_get_hbm_mode(void);
extern int sde_crtc_set_onscreenfinger_defer_sync(struct drm_crtc_state *crtc_state, bool defer_sync);
extern int oneplus_dimlayer_bl;
extern int oneplus_dimlayer_bl_enable_real;
extern int oneplus_dimlayer_bl_enable;
extern int oneplus_dimlayer_bl_enabled;
extern int oneplus_dimlayer_bl_delay;
extern int oneplus_dimlayer_bl_delay_after;
extern int oneplus_dimlayer_bl_enable_v2;
extern int oneplus_dimlayer_bl_enable_v3;
int oneplus_dimlayer_bl_enable_v2_real = 0;
extern int oneplus_dimlayer_bl_enable_v3_real;
extern int oneplus_datadimming_vblank_count;
extern atomic_t oneplus_datadimming_vblank_ref;
int oneplus_datadimming_v3_skip_frame = 2;
int sde_connector_update_backlight(struct drm_connector *connector)
{
	if (oneplus_dimlayer_bl != oneplus_dimlayer_bl_enabled) {
		struct sde_connector *c_conn = to_sde_connector(connector);

		oneplus_dimlayer_bl_enabled = oneplus_dimlayer_bl;
		usleep_range(oneplus_dimlayer_bl_delay, oneplus_dimlayer_bl_delay + 100);
		_sde_connector_update_bl_scale(c_conn);
		usleep_range(oneplus_dimlayer_bl_delay_after, oneplus_dimlayer_bl_delay_after + 100);
	}

	if (oneplus_dimlayer_bl_enable_v2 != oneplus_dimlayer_bl_enable_v2_real) {
		struct sde_connector *c_conn = to_sde_connector(connector);

		oneplus_dimlayer_bl_enable_v2_real = oneplus_dimlayer_bl_enable_v2;
		_sde_connector_update_bl_scale(c_conn);
	}

	if (oneplus_dimlayer_bl_enable_v3 != oneplus_dimlayer_bl_enable_v3_real) {
		struct sde_connector *c_conn = to_sde_connector(connector);

		if (oneplus_datadimming_v3_skip_frame > 0) {
			oneplus_datadimming_v3_skip_frame--;
		} else {
			oneplus_dimlayer_bl_enable_v3_real = oneplus_dimlayer_bl_enable_v3;
			_sde_connector_update_bl_scale(c_conn);
			oneplus_datadimming_v3_skip_frame=2;
		}
	}

	if (oneplus_datadimming_vblank_count> 0) {
		oneplus_datadimming_vblank_count--;
	} else {
		while (atomic_read(&oneplus_datadimming_vblank_ref) > 0) {
			drm_crtc_vblank_put(connector->state->crtc);
			atomic_dec(&oneplus_datadimming_vblank_ref);
		}
	}
...
extern int oneplus_dimlayer_hbm_vblank_count;
extern atomic_t oneplus_dimlayer_hbm_vblank_ref;
extern int oneplus_fod_on_vblank;
extern int oneplus_fod_off_vblank;
extern int oneplus_panel_update_backlight_unlock(struct dsi_panel *panel);
...
bool oneplus_skip_datadimming_sync = false;
extern int oneplus_display_mode;
extern ktime_t oneplus_backlight_time;
int sde_connector_update_hbm(struct drm_connector *connector)
{
	...
	if (fingerprint_mode != dsi_display->panel->is_hbm_enabled) {
		...
		if (oneplus_fod_on_vblank >= 0)
			panel->cur_mode->priv_info->fod_on_vblank = oneplus_fod_on_vblank;
		if (oneplus_fod_off_vblank >= 0)
			panel->cur_mode->priv_info->fod_off_vblank = oneplus_fod_off_vblank;
		...
		if (fingerprint_mode) {
			if (!dsi_display->panel->oneplus_priv.is_aod_ramless || oneplus_display_mode) {
				...
				if (oneplus_seed_backlight) {
					...
					oneplus_panel_process_dimming_v2(panel, panel->bl_config.bl_level, true);
					mipi_dsi_dcs_set_display_brightness(&panel->mipi_device, panel->bl_config.bl_level);
					oneplus_panel_process_dimming_v2_post(panel, true);
					usleep_range(frame_time_us, frame_time_us + 100);
				} else if (dsi_display->panel->oneplus_priv.is_aod_ramless) {
					ktime_t delta = ktime_sub(ktime_get(), oneplus_backlight_time);
			...
			oneplus_skip_datadimming_sync = true;
			oneplus_panel_update_backlight_unlock(panel);
			oneplus_skip_datadimming_sync = false;
			...
			if(ONEPLUS_DISPLAY_AOD_HBM_SCENE == get_oneplus_display_scene()) {
				if (ONEPLUS_DISPLAY_POWER_DOZE_SUSPEND == get_oneplus_display_power_status() ||
				    ONEPLUS_DISPLAY_POWER_DOZE == get_oneplus_display_power_status()) {
					rc = dsi_panel_tx_cmd_set(dsi_display->panel, DSI_CMD_AOD_HBM_OFF);
					oneplus_update_aod_light_mode_unlock(panel);
					set_oneplus_display_scene(ONEPLUS_DISPLAY_AOD_SCENE);
				} else {
					...
					if(panel->bl_config.bl_level > 1023)
						oneplus_panel_update_backlight_unlock(panel);
					if (oneplus_display_get_hbm_mode())
						rc = dsi_panel_tx_cmd_set(dsi_display->panel, DSI_CMD_AOD_HBM_ON);
					set_oneplus_display_scene(ONEPLUS_DISPLAY_NORMAL_SCENE);
				}
			} else if (oneplus_display_get_hbm_mode()) {
				/* Do nothing to skip hbm off */
			} else if(ONEPLUS_DISPLAY_AOD_SCENE == get_oneplus_display_scene()) {
				rc = dsi_panel_tx_cmd_set(dsi_display->panel, DSI_CMD_AOD_HBM_OFF);
				oneplus_update_aod_light_mode_unlock(panel);
			} else {
	...
	if (oneplus_dimlayer_hbm_vblank_count > 0) {
		oneplus_dimlayer_hbm_vblank_count--;
	} else {
		while (atomic_read(&oneplus_dimlayer_hbm_vblank_ref) > 0) {
			drm_crtc_vblank_put(connector->state->crtc);
			atomic_dec(&oneplus_dimlayer_hbm_vblank_ref);
		}
	}
