...
struct drm_msm_pcc oneplus_save_pcc;
bool oneplus_pcc_enabled = false;
bool oneplus_skip_pcc = false;
...
bool sde_cp_crtc_update_pcc(struct drm_crtc *crtc)
{
	...
	if (oneplus_skip_pcc == pcc_skip_mode)
		return false;

	oneplus_skip_pcc = pcc_skip_mode;
	memset(&hw_cfg, 0, sizeof(hw_cfg));

	if (!pcc_skip_mode && oneplus_pcc_enabled){
		hw_cfg.payload = &oneplus_save_pcc;
		hw_cfg.len = sizeof(oneplus_save_pcc);
	}
				   ...
				   struct sde_crtc *sde_crtc)
{
	...
	if (prop_node->feature == SDE_CP_CRTC_DSPP_PCC && is_dsi_panel(&sde_crtc->base)) {
		if (hw_cfg.payload && (hw_cfg.len == sizeof(oneplus_save_pcc))) {
			memcpy(&oneplus_save_pcc, hw_cfg.payload, hw_cfg.len);
			oneplus_pcc_enabled = true;

			if (is_skip_pcc(&sde_crtc->base)) {
				hw_cfg.payload = NULL;
				hw_cfg.len = 0;
				oneplus_skip_pcc = true;
			} else {
				oneplus_skip_pcc = false;
			}
		} else {
			oneplus_pcc_enabled = false;
		}
	}
