...
int oneplus_dimlayer_fingerprint_failcount = 0;
int oneplus_underbrightness_alpha = 0;
extern int oneplus_dfps_idle_off;
extern int msm_drm_notifier_call_chain(unsigned long val, void *v);
extern int oneplus_display_atomic_check(struct drm_crtc *crtc, struct drm_crtc_state *state);
		...
		if (cstate->fingerprint_dim_layer) {
			...
			drm_atomic_crtc_for_each_plane(plane, crtc) {
				...
				if (pstate->stage == cstate->fingerprint_dim_layer->stage) {
					is_dim_valid = false;
					oneplus_dimlayer_fingerprint_failcount++;
					SDE_ERROR("Skip fingerprint_dim_layer as it shared plane stage %d %d\n",
							pstate->stage, cstate->fingerprint_dim_layer->stage);
					SDE_EVT32(pstate->stage, cstate->fingerprint_dim_layer->stage, zpos_max, oneplus_dimlayer_fingerprint_failcount);
				}
			}
			if (is_dim_valid) {
				_sde_crtc_setup_dim_layer_cfg(crtc, sde_crtc,
...

	{
		...
		if (old_cstate->fingerprint_pressed != cstate->fingerprint_pressed) {
			...
			if (cstate->fingerprint_defer_sync) {
				u32 target_vblank = oneplus_onscreenfp_vblank_count + 1;
				 ...
				 */
				if (current_vblank == (oneplus_onscreenfp_vblank_count + 1)) {
					exp_ktime = ktime_add_ms(oneplus_onscreenfp_pressed_time, 4);
...
static int sde_crtc_config_fingerprint_dim_layer(struct drm_crtc_state *crtc_state, int stage)
{
	...
	oneplus_underbrightness_alpha = alpha;
...
extern int oneplus_dimlayer_hbm;
extern int oneplus_dimlayer_bl_alpha_value;
extern bool is_oneplus_aod_ramless(void);
extern bool is_dsi_panel(struct drm_crtc *crtc);
extern int oneplus_get_panel_brightness(void);
extern int oneplus_dimlayer_bl_enable;
extern bool oneplus_ffl_trigger_finish;
int oneplus_dimlayer_bl = 0;
extern ktime_t oneplus_backlight_time;
extern u32 oneplus_last_backlight;
extern u32 oneplus_backlight_delta;
		...
		struct plane_state *pstates, int cnt)
{
	...
	int dimlayer_hbm = oneplus_dimlayer_hbm;
	int dimlayer_bl = 0;
	int i;
	...
	if (oneplus_dimlayer_bl_enable) {
		int backlight = oneplus_get_panel_brightness();

		if (backlight > 1 && backlight < oneplus_dimlayer_bl_alpha_value &&
		    oneplus_ffl_trigger_finish == true) {
			ktime_t now = ktime_get();
			ktime_t delta = ktime_sub(now, oneplus_backlight_time);

			if (oneplus_backlight_delta > 9) {
				if (oneplus_dimlayer_bl == 0 && ktime_to_ns(delta) > 25000000)
					oneplus_dimlayer_bl = 1;
			} else {
				oneplus_dimlayer_bl = 1;
			}
			if (oneplus_dimlayer_bl)
				dimlayer_bl = 1;
		} else {
			oneplus_dimlayer_bl = 0;
		}
	} else {
		oneplus_dimlayer_bl = 0;
	}
	...
	if (dimlayer_hbm || dimlayer_bl) {
		...
		if (lcd_closebl_flag_fp) {
			oneplus_underbrightness_alpha = 0;
			cstate->fingerprint_dim_layer = NULL;
			cstate->fingerprint_mode = false;
			return 0;
		}
		...
		if (fppressed_index >= 0 && !(is_oneplus_aod_ramless() && cstate->base.mode.flags & DRM_MODE_FLAG_CMD_MODE_PANEL))
		...
		struct sde_multirect_plane_states *multirect_plane)
{
	...
	rc = oneplus_display_atomic_check(crtc, state);
	if (rc)
		return rc;
