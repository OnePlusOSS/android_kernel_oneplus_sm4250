...
/*xueqiang, 2020/06/15, add for modify spi speed*/
...
#else
	    if ((idev->ts->boot_mode == MSM_BOOT_MODE_FACTORY ||
		idev->ts->boot_mode == MSM_BOOT_MODE_RF))
#endif
	    {
            mutex_lock(&idev->touch_mutex);
            idev->actual_tp_mode = P5_X_FW_DEMO_MODE;
...
static int ilitek_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	case GESTURE_W:
		...
		break;
	case GESTURE_S:/*ritchie add for gesture s*/
		gesture->gesture_type  = Sgestrue;/*ritchie add for gesture s*/
		break;/*ritchie add for gesture s*/
                                  ...
                                  u8 read_type)
{
    ...
    enable_irq(idev->irq_num);//because oneplus disable
...
int __maybe_unused ilitek_platform_probe(struct spi_device *spi)
{
    ...
    /*for gesture ws*/
    idev->gesture_process_ws = wakeup_source_register(idev->dev, "gesture_wake_lock");
    if (!idev->gesture_process_ws) {
