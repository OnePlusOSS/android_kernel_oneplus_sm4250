...
static int ilitek_tddi_ic_check_support(u32 pid, u16 id)
{
    ...
    idev->chip->id = id;//for oneplus ftm mode
...
int ilitek_tddi_ic_check_busy(int count, int delay)
{
	...
	do {
		if (ilitek_thp_send_cmd(&cmd[1], sizeof(u8), busy, sizeof(busy)) < 0) {
			ipio_err("sem cmd failed");
			return -EINVAL;
		}
...
int ilitek_tddi_ic_get_fw_ver(void)
{
	...
	/*if (g_tp_dev_vendor == 1)
		snprintf(Ctp_name, 20, "HLT,ILI9881H\n");
	else if (g_tp_dev_vendor == 2)
		snprintf(Ctp_name, 20, "TXD,ILI9881H\n");
	else if (g_tp_dev_vendor == 4)
		snprintf(Ctp_name, 20, "TXD_BOE,ILI9881H\n");
	else
		TPD_INFO("Not fund touchscreen !");

	TPD_INFO(" Ctp_name is : %s\n", Ctp_name); */
#endif
	//snprintf(dev_version, MAX_DEVICE_VERSION_LENGTH, "%02X", buf[3]);
	//snprintf(Ctp_vendor, 10, "0x%x\n", buf[3]);
	//push_component_info(TP, Ctp_vendor, Ctp_name);
