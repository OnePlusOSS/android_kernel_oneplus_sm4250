...
static ssize_t ilitek_node_change_list_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "[Patch] add slave mode Ritchie 2020/06/29\n");
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
    ...
    if (strcmp(cmd, "hwreset") == 0) {
        ...
        TPD_INFO("oneplus kit ctrl this\n");
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_IOCTL_WRAPPER_RW:
		...
		if (ilitek_thp_send_cmd(szBuf + 4, wrap_wlen, wrap_rbuf, wrap_rlen) < 0)
			ipio_err("Failed to ilitek_thp_send_cmd\n");
...
int ilitek_create_proc_for_oneplus(struct touchpanel_data *ts)
{
    int ret = 0;
