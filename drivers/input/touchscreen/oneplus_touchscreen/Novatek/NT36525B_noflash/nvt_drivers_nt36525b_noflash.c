/*****************************************************************************************
 * Copyright (c)  2008- 2019  Oneplus Mobile communication Corp.ltd.
...
/********* Start of implementation of oneplus_touchpanel_operations callbacks********************/
...
int nvt_lcd_reset_gpio_init(void *chip_data)
{
    ...
    np = ts->dev->of_node;
    chip_info->lcd_reset_gpio = of_get_named_gpio(np, "lcd-reset-gpio", 1);
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    ...
    struct chip_data_nt36525b *chip_info = (struct chip_data_nt36525b *)chip_data;
    uint8_t point_data[POINT_DATA_LEN + 2] = {0};
    uint8_t max_num = 10;	//TODO: define max_num by oneplus common driver
...
static int nvt_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    /* Debug Function */
    /*case MODE_HOPPING_POLLING:	//TODO: add define in oneplus h file, MODE_HOPPING_POLLING
    ...
    case MODE_HOPPING_FIX_FREQ:	//TODO: add define in oneplus h file, MODE_HOPPING_FIX_FREQ
    ...
    /*case DEBUG_MODE_MESSAGE_DIFF:	//TODO: add define in oneplus h file, DEBUG_MODE_MESSAGE_DIFF
    ...
    case DEBUG_MODE_GESTURE_COORDINATE:	//TODO: add define in oneplus h file, DEBUG_MODE_GESTURE_COORDINATE
    ...
    case DEBUG_MODE_GESTURE_COORDINATE_RECORD:	//TODO: add define in oneplus h file, DEBUG_MODE_GESTURE_COORDINATE_RECORD
...
static void store_to_file(int fd, char *format, ...)
{
    ...
    if(fd >= 0) {
        //sys_write(fd, buf, strlen(buf));//by wenjie
    }
}
...
TEST_END:
    ...
    //sys_mkdir("/sdcard/TpTestReport/screenOff/NG", 0666);//by wenjie
    //fd = sys_open(data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);//by wenjie
    if (fd < 0) {
...
static struct oneplus_touchpanel_operations nvt_ops = {
...
static struct debug_info_proc_operations debug_info_proc_ops = {
    ...
    //.dbg_diff_finger_down_read = nvt_dbg_diff_finger_down_read, //TODO: link to oneplus common driver
    //.dbg_diff_status_change_read = nvt_dbg_diff_status_change_read, //TODO: link to oneplus common driver
    //.dbg_gesture_record_coor_read = nvt_dbg_gesture_record_coor_read, //TODO: link to oneplus common driver
...
TEST_END:
    ...
    //sys_mkdir("/sdcard/TpTestReport/screenOn/NG", 0666);//by wenjie
    //nvt_testdata->fd = sys_open(data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);//by wenjie
    ...
    /* Project Name */
    //store_to_file(nvt_testdata->fd, "ONEPLUS_%d\n", get_project());//wenjie
    /* Project ID */
...
OUT:
    if (nvt_testdata->fd >= 0) {
        //sys_close(nvt_testdata->fd);//by wenjie
    }
    set_fs(old_fs);
...
static void nova_init_oneplus_apk_op(struct touchpanel_data *ts)
{
...
#else
    if (/*ts->boot_mode == MSM_BOOT_MODE__RECOVERY ||*/ is_oem_unlocked() || ts->fw_update_in_probe_with_headfile)//wenjie
#endif
    {
