...
static ssize_t fts_hw_reset_node_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
...
static DEVICE_ATTR(fts_hw_reset_node, 0644, fts_hw_reset_node_show, NULL);
...
static DEVICE_ATTR(fts_irq, 0644, NULL, fts_irq_store);

static ssize_t fts_fw_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(fts_fw_version, 0644, fts_fw_version_show, NULL);

static ssize_t fts_rw_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;

	if (!g_rwreg_result.op) {
		if (g_rwreg_result.result == 0)
			count = snprintf(buf, sizeof(buf), "Read %02X: %02X\n",
			g_rwreg_result.reg, g_rwreg_result.value);
		else
			count = snprintf(buf, sizeof(buf), "Read %02X failed, ret: %d\n",
			g_rwreg_result.reg, g_rwreg_result.result);
	} else {
		if (g_rwreg_result.result == 0)
			count = snprintf(buf, sizeof(buf), "Write %02X, %02X success\n",
			g_rwreg_result.reg, g_rwreg_result.value);
		else
			count = snprintf(buf, sizeof(buf), "Write %02X failed, ret: %d\n",
			g_rwreg_result.reg, g_rwreg_result.result);
	}
...
static ssize_t fts_rw_reg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct touchpanel_data *ts = dev_get_drvdata(dev);
	struct focal_debug_func *focal_debug_ops = (struct focal_debug_func *)ts->private_data;
	ssize_t num_read_chars = 0;
...
static DEVICE_ATTR(fts_rw_reg, 0644, fts_rw_reg_show, fts_rw_reg_store);

static ssize_t fts_esd_check_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct touchpanel_data *ts = dev_get_drvdata(dev);
...
static ssize_t fts_esd_check_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct touchpanel_data *ts = dev_get_drvdata(dev);
	struct focal_debug_func *focal_debug_ops = (struct focal_debug_func *)ts->private_data;
	...
	if (ts->esd_handle_support) {
		if (focal_debug_ops && focal_debug_ops->get_esd_check_flag) {
			count = snprintf(buf, sizeof(buf), "esd_running_flag: %d,  esd_check_need_stop: %d\n",
				ts->esd_info.esd_running_flag, focal_debug_ops->get_esd_check_flag(ts->chip_data));
		} else {
			count = snprintf(buf, sizeof(buf), "esd_running_flag: %d\n", ts->esd_info.esd_running_flag);
		}
	} else {
		count = snprintf(buf, sizeof(buf), "not support esd handle\n");
	}

	return count;
}
static DEVICE_ATTR(fts_esd_check, 0644, fts_esd_check_show, fts_esd_check_store);

static ssize_t fts_dump_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char tmp[256];
	int count = 0;
	struct touchpanel_data *ts = dev_get_drvdata(dev);
...
static DEVICE_ATTR(fts_dump_reg, 0644, fts_dump_reg_show, NULL);
...
static struct attribute *focal_attributes[] = {
	...
	&dev_attr_fts_hw_reset_node.attr,
...
unsigned char proc_operate_mode;
...
int focal_create_apk_debug_channel(struct touchpanel_data *ts)
{
	...
	focal_proc_entry = proc_create_data("ftxxxx-debug", 0664, NULL, &focal_proc_fops, ts);
	if (focal_proc_entry == NULL) {
		ret = -ENOMEM;
...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (buflen <= 1) {
		TPD_INFO("apk proc write count(%d) fail", buflen);
		return -EINVAL;
	}
	...
	if (buflen > PROC_BUF_SIZE) {
		...
		if (writebuf == NULL) {
			TPD_INFO("apk proc write buf zalloc fail");
			return -ENOMEM;
		}
	} else {
		writebuf = tmpbuf;
...
static int fts_auto_test_read_func(struct seq_file *s, void *v)
{
	...
	if (ts->suspend_state != TP_SPEEDUP_RESUME_COMPLETE) {
		seq_puts(s, "Not in resume state\n");
		return 0;
	}
...
int fts_create_proc(struct touchpanel_data *ts, struct fts_proc_operations *syna_ops)
{
	...
	g_syna_ops = syna_ops;
	prEntry_tmp = proc_create_data("baseline_test", 0664, ts->prEntry_tp, &fts_auto_test_proc_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}

	prEntry_tmp = proc_create_data("ctp_openshort_test", 0664, ts->prEntry_tp, &fts_auto_test_proc_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}
...
void ft_limit_read_std(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	if (ret < 0) {
		TPD_INFO("Request firmware failed - %s (%d)\n", ts->panel_data.test_limit_name, ret);
		seq_puts(s, "Request failed, Check the path\n");
		return;
	}
	...
	if ((ph->magic1 != LIMIT_HEADER_MAGIC_1) || (ph->magic2 != LIMIT_HEADER_MAGIC_2)) {
		...
		seq_puts(s, "limit image is not generated by oplus\n");
		release_firmware(fw);
		return;
	}
	...
	if (!item_cnt) {
		...
		seq_puts(s, "limit image has no test item\n");
	}
	...
	for (m = 0; m < item_cnt; m++) {
		...
		if (item_head->item_limit_type == LIMIT_TYPE_TOP_FLOOR_RX_TX_DATA) {
			if (item_head->item_bit == TYPE_SCAP_CB_DATA)
				...
				seq_puts(s, "TYPE_OPEN_RAWDATA:\n");

			TPD_INFO("top data [%d]:\n", m);
			seq_puts(s, "water proof mode:\n");
			p_data32 = (int32_t *)(fw->data + item_head->top_limit_offset);
