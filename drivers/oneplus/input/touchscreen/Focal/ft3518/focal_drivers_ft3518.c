...
struct fts_ts_data *focal_data;

/*******Part0:LOG TAG Declear********************/

#define TPD_DEVICE "focaltech,focal"
...
static int focal_dump_reg_state(void *chip_data, char *buf)
{
	...
	count += snprintf(buf + count, sizeof(buf + count), "Power Mode:0x%02x\n", regvalue);

	//FW version
	regvalue = touch_i2c_read_byte(ts_data->client, FTS_REG_FW_VER);
	count += snprintf(buf + count, sizeof(buf + count), "FW Ver:0x%02x\n", regvalue);

	//Vendor ID
	regvalue = touch_i2c_read_byte(ts_data->client, FTS_REG_VENDOR_ID);
	count += snprintf(buf + count, sizeof(buf + count), "Vendor ID:0x%02x\n", regvalue);

	// 1 Gesture mode,0 Normal mode
	regvalue = touch_i2c_read_byte(ts_data->client, FTS_REG_GESTURE_EN);
	count += snprintf(buf + count, sizeof(buf + count), "Gesture Mode:0x%02x\n", regvalue);

	// 3 charge in
	regvalue = touch_i2c_read_byte(ts_data->client, FTS_REG_CHARGER_MODE_EN);
	count += snprintf(buf + count, sizeof(buf + count), "charge stat:0x%02x\n", regvalue);

	//Interrupt counter
	regvalue = touch_i2c_read_byte(ts_data->client, FTS_REG_INT_CNT);
	count += snprintf(buf + count, sizeof(buf + count), "INT count:0x%02x\n", regvalue);

	//Flow work counter
	regvalue = touch_i2c_read_byte(ts_data->client, FTS_REG_FLOW_WORK_CNT);
	count += snprintf(buf + count, sizeof(buf + count), "ESD count:0x%02x\n", regvalue);
...
static int fts_enable_black_gesture(struct fts_ts_data *ts_data, bool enable)
{
	...
	if (enable) {
		touch_i2c_write_byte(ts_data->client, 0xD1, 0XFF);
		touch_i2c_write_byte(ts_data->client, 0xD2, 0XFF);
		touch_i2c_write_byte(ts_data->client, 0xD5, 0XFF);
		touch_i2c_write_byte(ts_data->client, 0xD6, 0XFF);
		touch_i2c_write_byte(ts_data->client, 0xD7, 0XFF);
		touch_i2c_write_byte(ts_data->client, 0xD8, 0XFF);
	}
	return touch_i2c_write_byte(ts_data->client, FTS_REG_GESTURE_EN, enable);
}

static int fts_edge_limit_switch(struct fts_ts_data *ts_data, bool enable)
{
	u8 edge_mode = 0;
	...
	TPD_INFO("MODE_LIMIT_SWITCH, write 0x8C=%d\n", edge_mode);
...
static int fts_enable_edge_limit(struct fts_ts_data *ts_data, bool enable)
{
	...
	/*0:Horizontal, 1:Vertical*/
	if ((enable == 1) || (ts_data->touch_direction == VERTICAL_SCREEN)) {
		edge_mode = 0;
	} else if (enable == 0) {
		if (ts_data->touch_direction == LANDSCAPE_SCREEN_90)
			edge_mode = 1;
		else if (ts_data->touch_direction == LANDSCAPE_SCREEN_270)
			edge_mode = 2;
	}
...
static int fts_enable_gesture_mask(struct fts_ts_data *ts_data, bool enable)
{
	int ret = -1;

	TPD_INFO("%s, enable = %d\n", __func__, enable);
	if (enable) {
		ret = touch_i2c_write_byte(ts_data->client, FTS_REG_POWER_MODE, 0x03);/*mask all gestures */
		TPD_INFO("MODE_SLEEP, write 0xA5=3\n");
	} else {
		if (ts_data->ts->is_suspended) {	// do not pull up reset when doing resume
			fts_hw_reset(ts_data, RESET_TO_NORMAL_TIME);
		}
		ret = fts_enable_black_gesture(ts_data, true);/* enable all gesture */
	}
...
static int fts_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_GESTURE_SWITCH:
		ret = fts_enable_gesture_mask(ts_data, flag);
		if (ret < 0) {
			TPD_INFO("%s: enable gesture mask failed.\n", __func__);
			goto mode_err;
		}
		break;
	...
	case MODE_LIMIT_SWITCH:
		ret = fts_edge_limit_switch(ts_data, flag);
		if (ret < 0) {
	...
	case MODE_TOUCH_HOLD:
		fts_enable_fingerprint_underscreen(ts_data, flag);
		break;
...
static int fts_get_vendor(void *chip_data, struct panel_info *panel_data)
{
	...
	//panel_data->fw_name = "FW_AA411.bin";
	TPD_INFO("tp_type = %d, panel_data->fw_name = %s\n", panel_data->tp_type, panel_data->fw_name);
...
static fw_check_state fts_fw_check(
	void *chip_data,
	...
	struct panel_info *panel_data)
{
	...
	snprintf(Ctp_name, sizeof(Ctp_name), "SAMSUNG,FT3518,FW:0x%x\n", panel_data->TP_FW);
	#endif
	...
	if (panel_data->manufacture_info.version) {
		...
		snprintf(panel_data->manufacture_info.version, MAX_DEVICE_VERSION_LENGTH, "0x%x", panel_data->TP_FW);
...
static u32 fts_u32_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
	...
	TPD_DEBUG("%s, fgerprint, touched = %d, event_type = %d, fp_down = %d, fp_down_report = %d, ",
		__func__,
		ts_data->ts->view_area_touched,
		ts_data->fod_info.event_type,
	...
	if (!is_suspended && ts_data->fp_en) {
		...
		if (!g_tp->touchold_event)
			ts_data->fod_info.fp_down_report = g_tp->touchold_event;
	...
	case GESTURE_FINGER_PRINT:
		...
		if (ts_data->fod_info.event_type == FTS_EVENT_FOD) {
			if (ts_data->fod_info.fp_down && !ts_data->fod_info.fp_down_report) {
				...
				ts_data->fod_info.fp_down_report = 1;
				tp_info.x = ts_data->fod_info.fp_x;
				tp_info.y = ts_data->fod_info.fp_y;
...
static void fts_enable_fingerprint_underscreen(void *chip_data, uint32_t enable)
{
	...
	TPD_DEBUG("%s,touched = %d, event_type = %d, fp_down = %d. fp_down_report = %d\n",
		__func__,
		ts_data->ts->view_area_touched,
		ts_data->fod_info.event_type,
		ts_data->fod_info.fp_down,
	...
	if (enable) {
		...
		if ((!ts_data->ts->view_area_touched) &&
			...
			ts_data->fod_info.fp_down_report = 0;
			TPD_DEBUG("%s, fgerprint, fp_down_report status abnormal, needed to be reseted!\n",
				__func__);
		}
	} else {
		val &= 0xFD;
...
static struct touchpanel_operations fts_ops = {
	...
	.gt_health_report			= fts_health_report,
