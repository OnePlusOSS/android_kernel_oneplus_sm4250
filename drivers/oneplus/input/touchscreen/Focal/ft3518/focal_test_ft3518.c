...
/* Focaltek Android Driver Sample Code for common functions
 *
 ...
 * ---------------- Revision History: --------------------------
 *	 <version>		  <date>				  < author >
 * Revision 1.1, 2016-09-09, Tong.han@Bsp.Group.Tp,
 * modify based on gerrit review result(http://gerrit.scm.adc.com:8080/#/c/223721/)
 */
...
#define FTS_TEST_FUNC_ENTER() TPD_INFO("[FTS_TS][TEST]%s: Enter\n", __func__)

#define FTS_TEST_FUNC_EXIT() TPD_INFO("[FTS_TS][TEST]%s: Exit(%d)\n", __func__, __LINE__)

#define FTS_TEST_SAVE_INFO(fmt, args...) TPD_INFO(fmt, ##args)

#define FTS_TEST_SAVE_ERR(fmt, args...) TPD_INFO(fmt, ##args)

/*#define FTS_TEST_SAVE_INFO(fmt, args...) do { \
 *	if (focal_data->s) { \
 *		seq_printf(focal_data->s, fmt, ##args); \
 *	} \
 *	TPD_INFO(fmt, ##args); \
 *} while (0)
 *
...
static int fts_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
	...
	ret = touch_i2c_read(focal_data->client, (char *)cmd, cmdlen, (char *)data, datalen);
	if (ret < 0)
		return ret;
	else
		return 0;
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
	...
	ret = touch_i2c_write(focal_data->client, (char *)writebuf, writelen);
	if (ret < 0)
		return ret;
	else
		return 0;
...
static int read_mass_data(u8 addr, int byte_num, int *buf)
{
	...
	data = kcalloc(byte_num, sizeof(u8), GFP_KERNEL);
	if (data == NULL) {
...
static int fts_special_operation_for_samsung(struct fts_ts_data *ts_data)
{
	...
	if (ts_data->use_panelfactory_limit == true) {/*only for firmware released to samsung factory*/
...
static int enter_factory_mode(struct fts_ts_data *ts_data)
{
	...
	ret = fts_test_read_reg(DEVIDE_MODE_ADDR, &mode);
	if ((ret >= 0) && (mode == FTS_FACTORY_MODE)) {
...
static int get_cb_sc(int byte_num, int *cb_buf, enum byte_mode mode)
{
	...
	cb = kcalloc(byte_num, sizeof(u8), GFP_KERNEL);
	if (!cb) {
...
static bool compare_mc_sc(struct fts_ts_data *ts_data, bool tx_check, bool rx_check, int *data, int *min, int *max)
{
	...
	if (rx_check) {
		for (i = 0; i < rx_num; i++) {
			if (ts_data->mpt.thr.node_valid_sc[i] == 0)
				continue;
...
static int short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		if ((ret >= 0) && (retval == short_state))
			break;
		TPD_DEBUG("reg%x=%x,retry:%d", FACTROY_REG_SHORT_TEST_EN, short_state, i);
	}
...
static bool short_test_get_channel_num(
...
static bool short_test_channel_to_channel_chonse(u8 *error_ch, u8 *min_70k_ch, int min_70k_num, int count)
{
	int num;
	bool is_used = false;

	for (num = 0; num < (min_70k_num); num++) {
		if (error_ch[count] == min_70k_ch[num]) {
			is_used = true;
			break;
		}
	}
...
static bool short_test_channel_to_gnd(
	struct fts_ts_data *ts_data,
	int *fm_short_resistance,
	int *adc_data,
	int offset,
	bool *is_weak_short_gnd)
{
	...
	error_ch = kcalloc(total_num, sizeof(u8), GFP_KERNEL);
	if (error_ch == NULL) {
		FTS_TEST_SAVE_ERR("error_ch buffer malloc fail\n");
		goto TEST_END;
	}
	min_70k_ch = kcalloc(total_num, sizeof(u8), GFP_KERNEL);
	if (min_70k_ch == NULL) {
		FTS_TEST_SAVE_ERR("min_70k_ch buffer malloc fail\n");
		goto TEST_END;
	}

	w_buf = kcalloc((total_num + 3), sizeof(u8), GFP_KERNEL);
	if (w_buf == NULL) {
		FTS_TEST_SAVE_ERR("w_buf buffer malloc fail\n");
		goto TEST_END;
	}

	tmp_adc_data = kcalloc(total_num * total_num, sizeof(int), GFP_KERNEL);
	if (tmp_adc_data == NULL) {
		FTS_TEST_SAVE_ERR("adc_data buffer malloc fail\n");
		goto TEST_END;
	}

	fg_short_resistance =  kcalloc(total_num, sizeof(int), GFP_KERNEL);
		...
		for (i = 0; i < error_num; i++) {
			...
			if (min_cg > fvalue) {
				...
				*is_weak_short_gnd = true;
				...
				if (fvalue < 70) {
					is_used = false;
					is_used = short_test_channel_to_channel_chonse(error_ch,
						min_70k_ch, min_70k_num, i);
					if (!is_used) {
...
static bool short_test_channel_to_channel
(struct fts_ts_data *ts_data, int *fm_short_resistance, int *adc_data, int offset)
{
	int ret = 0;
	int error_num = 0;
	int min_70k_num = 0;
	...
	f_origin_resistance = kcalloc(total_num, sizeof(int), GFP_KERNEL);
	...
	if (error_num > 1) {
		...
		ret = fts_test_write(FACTROY_REG_SHORT_AB_CH, w_buf, error_num + 1);
		for (j = 0; j < 1; j++) {
			ret = short_get_adc_data_mc(TEST_RETVAL_00, error_num *
				(error_num - 1) * 2 / 2, tmp_adc_data, FACTROY_REG_SHORT_CC);
			sys_delay(50);
			if (ret) {
				FTS_TEST_SAVE_ERR("failed to get weak short data,ret= %d\n", ret);
				tmp_result = false;
		...
		for (i = 0; i < error_num; i++) {
			for (j = i + 1; j < error_num; j++) {
				...
				if (min_cc > fvalue) {
					...
					if ((fvalue > 70) && (error_ch[i] <= tx_num))
	...
	if (min_70k_num > 0) {
		...
		memset(tmp_adc_data, 0, (all_adc_data_num + 1));
		for (i = 0; i < 1; i++) {
			ret = short_get_adc_data_mc(TEST_RETVAL_00, min_70k_num *
				(min_70k_num - 1) * 2 / 2, tmp_adc_data, FACTROY_REG_SHORT_CC);
			sys_delay(50);
			if (ret) {
				FTS_TEST_SAVE_ERR("failed to get weak short data,ret= %d\n", ret);
				tmp_result = false;
		...
		for (i = 0; i < min_70k_num; i++) {
			for (j = i + 1; j < min_70k_num; j++) {
				...
				fvalue =
					(tmp_adc_data[adc_count] - offset + 395) *
...
static int fts_auto_preoperation(struct fts_ts_data *ts_data)
{
	...
	ts_data->noise_rawdata = kcalloc(node_num, sizeof(int), GFP_KERNEL);
	if (!ts_data->noise_rawdata) {
		FTS_TEST_SAVE_ERR("kzalloc for noise_rawdata fail\n");
		goto alloc_err;
	}

	ts_data->rawdata = kcalloc(node_num, sizeof(int), GFP_KERNEL);
	if (!ts_data->rawdata) {
	...
	ts_data->scap_cb = kcalloc(channel_num * NUM_MODE, sizeof(int), GFP_KERNEL);
	if (!ts_data->scap_cb) {
		FTS_TEST_SAVE_ERR("kzalloc for scap_cb fail\n");
		goto alloc_err;
	}

	ts_data->scap_rawdata = kcalloc(channel_num * NUM_MODE, sizeof(int), GFP_KERNEL);
	if (!ts_data->scap_rawdata) {
		FTS_TEST_SAVE_ERR("kzalloc for scap_rawdata fail\n");
		goto alloc_err;
	}

	ts_data->panel_differ_raw = kcalloc(node_num, sizeof(int), GFP_KERNEL);
	if (!ts_data->panel_differ_raw) {
		FTS_TEST_SAVE_ERR("kzalloc for panel_differ_raw fail\n");
		goto alloc_err;
	}

	ts_data->rawdata_linearity = kcalloc(node_num * 2, sizeof(int), GFP_KERNEL);
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((ts_data->rawdata[i] < ts_data->fts_autotest_offset->fts_raw_data_N[i]) ||
			...
			TPD_INFO("raw data ERR [%d]: [%d] > [%d] > [%d]\n",
				i, ts_data->fts_autotest_offset->fts_raw_data_P[i],
				ts_data->rawdata[i],
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((rl_tmp[i] < ts_data->fts_autotest_offset->fts_uniformity_data_N[i]) ||
			...
			TPD_INFO("uniformity data ERR [%d]: [%d] > [%d] > [%d]\n",
				...
				rl_tmp[i], ts_data->fts_autotest_offset->fts_uniformity_data_N[i]);
			FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
...
static int fts_scap_cb_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->fts_autotest_offset->fts_scap_cb_data_P ||
		...
		TPD_INFO("fts_scap_cb_data_P || data_N || waterproof_N || waterproof_P is NULL");
		return 0;
	}
...
static int fts_scap_rawdata_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->fts_autotest_offset->fts_scap_raw_data_P ||
		...
		TPD_INFO("data_P || data_N ||waterproof_data_N || waterproof_data_P is NULL");
		return 0;
	}
...
static int fts_short_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	tmp_result = short_test_get_channel_num(ts_data,
	fm_short_resistance, adc_data, offset, &is_weak_short_mut);

	/* use the exceptional channel to conduct channel to ground short circuit test. */
	if (is_weak_short_mut)
		tmp_result = short_test_channel_to_gnd(ts_data,
		fm_short_resistance, adc_data, offset, &is_weak_short_gnd);

	/* use the exceptional channel to conduct channel to channel short circuit test. */
	if (is_weak_short_mut)
		tmp_result &= short_test_channel_to_channel(ts_data,
		fm_short_resistance, adc_data, offset);
...
static void fts_auto_write_result(struct fts_ts_data *ts_data, int failed_count)
{
	...
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "Noise Test",
		14, tx_num, rx_num, 11, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "Rawdata Test",
		7, tx_num, rx_num, 11 + tx_num, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "Rawdata Uniformity Test",
		16, tx_num, rx_num, 11 + tx_num * 2, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "Rawdata Uniformity Test",
		16, tx_num, rx_num, 11 + tx_num * 3, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "SCAP CB Test",
		9, 2, rx_num, 11 + tx_num * 4, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "SCAP CB Test",
		9, 2, rx_num, 11 + tx_num * 4 + 2, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "SCAP Rawdata Test",
		10, 2, rx_num, 11 + tx_num * 4 + 4, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "SCAP Rawdata Test",
		10, 2, rx_num, 11 + tx_num * 4 + 6, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	...
	fts_test_save_data("Rawdata Uniformity Test",
		ts_data->rawdata_linearity, ts_data->rl_cnt, rx_num, ts_data->csv_fd);
//	  fts_test_save_data("SCAP CB Test", ts_data->scap_cb, ts_data->scb_cnt, channel_num, ts_data->csv_fd);
	...
	fts_test_save_data("SCAP CB Test",
		ts_data->scap_cb + channel_num, ts_data->scb_cnt - channel_num, rx_num, ts_data->csv_fd);
//	  fts_test_save_data("SCAP Rawdata Test",
//		ts_data->scap_rawdata, ts_data->srawdata_cnt, channel_num, ts_data->csv_fd);
	fts_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata, channel_num, rx_num, ts_data->csv_fd);
	fts_test_save_data("SCAP Rawdata Test",
		ts_data->scap_rawdata + channel_num, ts_data->srawdata_cnt - channel_num, rx_num, ts_data->csv_fd);
...
static int fts_threshold_malloc(struct fts_ts_data *ts_data)
{
	...
	thr->scap_rawdata_on_min = kcalloc(channel_num, sizeof(int), GFP_KERNEL);
	if (!thr->scap_rawdata_on_min) {
		FTS_TEST_SAVE_ERR("kzalloc for scap_rawdata_on_min fail\n");
		goto thr_free;
	}

	thr->panel_differ_max = kcalloc(node_num, sizeof(int), GFP_KERNEL);
	if (!thr->panel_differ_max) {
		FTS_TEST_SAVE_ERR("kzalloc for panel_differ_max fail\n");
		goto thr_free;
	}

	thr->panel_differ_min = kcalloc(node_num, sizeof(int), GFP_KERNEL);
			...
			if (false == ts_data->use_panelfactory_limit) {
				...
				if (item_head->item_bit == TYPE_NOISE_DATA) {
					ts_data->fts_autotest_offset->fts_noise_data_P =
						(int32_t *)(limit_fw->data + item_head->top_limit_offset);
					ts_data->fts_autotest_offset->fts_noise_data_N =
					(int32_t *)(limit_fw->data + item_head->floor_limit_offset);
