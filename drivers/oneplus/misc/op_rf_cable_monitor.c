...
#elif defined OEM_TARGET_PRODUCT_BILLIE
...
int get_all_gpio_val(void)
{
	...
	if(get_rf_version() == 0) {
		for(i = 0; i< 3; i++)
			gpiostate = gpiostate + (gpio_get_value(rf_cable_data->cable_gpio[i]) * local_pow(10, 3 - i - 1));
	} else if (get_rf_version() == 8 || get_rf_version() == 12) {
