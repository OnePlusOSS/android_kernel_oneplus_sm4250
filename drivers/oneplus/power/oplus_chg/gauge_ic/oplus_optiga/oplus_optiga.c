...
void show_Puids_info(S_OPTIGA_PUID *puids, int length) {
	int i;

	for (i = 0; i < length; ++i) {
		printk(KERN_ERR "%s:[%d] 0x%X 0x%X 0x%X 0x%X\n", __func__, i,
			puids[i].uwVendorId, puids[i].uwProductId, puids[i].ulIdHigh, puids[i].ulIdLow);
	}
}
...
/*	set_pin_dir(1);
	...
	if (Swi_ReadUID((uint8_t *) &stDetectedPuids[0]) == FALSE) {
		...
		show_Puids_info(stDetectedPuids, MAX_DEV);
		return ret;
	}
	Swi_SetAddress(1);
