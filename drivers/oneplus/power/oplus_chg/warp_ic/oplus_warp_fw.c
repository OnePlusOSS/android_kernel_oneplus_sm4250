...
void oplus_warp_fw_type_dt(struct oplus_warp_chip *chip)
{
	...
	chip->support_plat_fg = of_property_read_bool(node, "qcom,support-platform-fg");
	chg_debug("oplus_warp_fw_type_dt support_plat_fg is %d\n", chip->support_plat_fg);
	...
	rc = of_property_read_u32(node, "qcom,warp_strategy1_batt_high_temp3",
		&chip->warp_strategy1_batt_high_temp3);
	if (rc) {
		chip->warp_strategy1_batt_high_temp3  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_batt_high_temp3 is %d\n",
			chip->warp_strategy1_batt_high_temp3);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_batt_high_temp4",
		&chip->warp_strategy1_batt_high_temp4);
	if (rc) {
		chip->warp_strategy1_batt_high_temp4  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_batt_high_temp4 is %d\n",
			chip->warp_strategy1_batt_high_temp4);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_batt_low_temp3",
		&chip->warp_strategy1_batt_low_temp3);
	if (rc) {
		chip->warp_strategy1_batt_low_temp3  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_batt_low_temp3 is %d\n",
			chip->warp_strategy1_batt_low_temp3);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_batt_low_temp4",
		&chip->warp_strategy1_batt_low_temp4);
	if (rc) {
		chip->warp_strategy1_batt_low_temp4  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_batt_low_temp4 is %d\n",
			chip->warp_strategy1_batt_low_temp4);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_high_current3",
		&chip->warp_strategy1_high_current3);
	if (rc) {
		chip->warp_strategy1_high_current3  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy1_high_current3 is %d\n",
			chip->warp_strategy1_high_current3);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_high_current4",
		&chip->warp_strategy1_high_current4);
	if (rc) {
		chip->warp_strategy1_high_current4  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy1_high_current4 is %d\n",
			chip->warp_strategy1_high_current4);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_high_current5",
		&chip->warp_strategy1_high_current5);
	if (rc) {
		chip->warp_strategy1_high_current5  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy1_high_current5 is %d\n",
			chip->warp_strategy1_high_current5);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_high_temp0",
		&chip->warp_strategy2_batt_high_temp0);
	if (rc) {
		chip->warp_strategy2_batt_high_temp0  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_high_temp0 is %d\n",
			chip->warp_strategy2_batt_high_temp0);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_high_temp1",
		&chip->warp_strategy2_batt_high_temp1);
	if (rc) {
		chip->warp_strategy2_batt_high_temp1  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_high_temp1 is %d\n",
			chip->warp_strategy2_batt_high_temp1);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_high_temp2",
		&chip->warp_strategy2_batt_high_temp2);
	if (rc) {
		chip->warp_strategy2_batt_high_temp2  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_high_temp2 is %d\n",
			chip->warp_strategy2_batt_high_temp2);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_high_temp3",
		&chip->warp_strategy2_batt_high_temp3);
	if (rc) {
		chip->warp_strategy2_batt_high_temp3  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_high_temp3 is %d\n",
			chip->warp_strategy2_batt_high_temp3);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_high_temp4",
		&chip->warp_strategy2_batt_high_temp4);
	if (rc) {
		chip->warp_strategy2_batt_high_temp4  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_high_temp4 is %d\n",
			chip->warp_strategy2_batt_high_temp4);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_low_temp0",
		&chip->warp_strategy2_batt_low_temp0);
	if (rc) {
		chip->warp_strategy2_batt_low_temp0  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_low_temp0 is %d\n",
			chip->warp_strategy2_batt_low_temp0);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_low_temp1",
		&chip->warp_strategy2_batt_low_temp1);
	if (rc) {
		chip->warp_strategy2_batt_low_temp1  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_low_temp1 is %d\n",
			chip->warp_strategy2_batt_low_temp1);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_low_temp2",
		&chip->warp_strategy2_batt_low_temp2);
	if (rc) {
		chip->warp_strategy2_batt_low_temp2  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_low_temp2 is %d\n",
			chip->warp_strategy2_batt_low_temp2);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_low_temp3",
		&chip->warp_strategy2_batt_low_temp3);
	if (rc) {
		chip->warp_strategy2_batt_low_temp3  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_low_temp3 is %d\n",
			chip->warp_strategy2_batt_low_temp3);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_batt_low_temp4",
		&chip->warp_strategy2_batt_low_temp4);
	if (rc) {
		chip->warp_strategy2_batt_low_temp4  = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy2_batt_low_temp4 is %d\n",
			chip->warp_strategy2_batt_low_temp4);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_high_current0",
		&chip->warp_strategy2_high_current0);
	if (rc) {
		chip->warp_strategy2_high_current0  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy2_high_current0 is %d\n",
			chip->warp_strategy2_high_current0);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_high_current1",
		&chip->warp_strategy2_high_current1);
	if (rc) {
		chip->warp_strategy2_high_current1  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy2_high_current1 is %d\n",
			chip->warp_strategy2_high_current1);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_high_current2",
		&chip->warp_strategy2_high_current2);
	if (rc) {
		chip->warp_strategy2_high_current2  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy2_high_current2 is %d\n",
			chip->warp_strategy2_high_current2);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_high_current3",
		&chip->warp_strategy2_high_current3);
	if (rc) {
		chip->warp_strategy2_high_current3  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy2_high_current3 is %d\n",
			chip->warp_strategy2_high_current3);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_high_current4",
		&chip->warp_strategy2_high_current4);
	if (rc) {
		chip->warp_strategy2_high_current4  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy2_high_current4 is %d\n",
			chip->warp_strategy2_high_current4);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy2_high_current5",
		&chip->warp_strategy2_high_current5);
	if (rc) {
		chip->warp_strategy2_high_current5  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy2_high_current5 is %d\n",
			chip->warp_strategy2_high_current5);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_high_temp1",
		&chip->warp_strategy1_skin_high_temp1);
	if (rc) {
		chip->warp_strategy1_skin_high_temp1 = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_skin_high_temp1 is %d\n",
			chip->warp_strategy1_skin_high_temp1);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_high_temp2",
		&chip->warp_strategy1_skin_high_temp2);
	if (rc) {
		chip->warp_strategy1_skin_high_temp2 = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_skin_high_temp2 is %d\n",
			chip->warp_strategy1_skin_high_temp2);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_low_temp1",
		&chip->warp_strategy1_skin_low_temp1);
	if (rc) {
		chip->warp_strategy1_skin_low_temp1 = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_skin_low_temp1 is %d\n",
			chip->warp_strategy1_skin_low_temp1);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_low_temp2",
		&chip->warp_strategy1_skin_low_temp2);
	if (rc) {
		chip->warp_strategy1_skin_low_temp2 = chip->warp_multistep_initial_batt_temp;
	} else {
		chg_debug("qcom,warp_strategy1_skin_low_temp2 is %d\n",
			chip->warp_strategy1_skin_low_temp2);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_current1",
		&chip->warp_strategy1_skin_current1);
	if (rc) {
		chip->warp_strategy1_skin_current1  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy1_skin_current1 is %d\n",
			chip->warp_strategy1_skin_current1);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_current2",
		&chip->warp_strategy1_skin_current2);
	if (rc) {
		chip->warp_strategy1_skin_current2  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy1_skin_current2 is %d\n",
			chip->warp_strategy1_skin_current2);
	}

	rc = of_property_read_u32(node, "qcom,warp_strategy1_skin_current3",
		&chip->warp_strategy1_skin_current3);
	if (rc) {
		chip->warp_strategy1_skin_current3  = chip->warp_strategy_normal_current;
	} else {
		chg_debug("qcom,warp_strategy1_skin_current3 is %d\n",
			chip->warp_strategy1_skin_current3);
	}
	...
	if (rc) {
		chip->warp_little_cool_bat_volt  = -1000;
	} else {
	...
	if (rc) {
		chip->warp_normal_bat_volt  = -1000;
	} else {
	...
	if (rc) {
		chip->warp_cool_bat_suspend_volt  = -1000;
	} else {
		chg_debug("qcom,warp_cool_bat_suspend_volt is %d\n",
			chip->warp_cool_bat_suspend_volt);
	}

	rc = of_property_read_u32(node, "qcom,warp_little_cool_bat_suspend_volt",
		&chip->warp_little_cool_bat_suspend_volt);
	if (rc) {
		chip->warp_little_cool_bat_suspend_volt  = -1000;
	} else {
	...
	if (rc) {
		chip->warp_normal_bat_suspend_volt  = -1000;
	} else {
	...
	if (rc) {
		chip->warp_warm_bat_suspend_volt  = -1000;
	} else {
...
#ifdef OEM_TARGET_PRODUCT_EBBA
		chg_err("chg_type unknown return false\n");
#endif
		return false;
	}
	if (oplus_warp_get_fastchg_to_normal() == true) {
