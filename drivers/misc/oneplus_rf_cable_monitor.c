...
static int op_rf_cable_probe(struct platform_device *pdev)
{
    ...
    ret = qcom_smem_alloc(QCOM_SMEM_HOST_ANY, SMEM_PROJECT_INFO, len);
    if (ret < 0 && ret != -EEXIST) {
        pr_err("%s smem_alloc fail\n", __func__);
        return -EFAULT;
    }
    the_rf_format = (struct rf_info_type*)qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_PROJECT_INFO, &smem_size);
	...
	if (rf_cable_0_support || rf_cable_1_support)
		proc_create("rf_cable", S_IRUGO, op_rf, &cable_proc_fops_cable);


    if (rf_pds_0_support || rf_pds_1_support){
        proc_create("rf_pds", S_IRUGO, op_rf, &cable_proc_fops_pds);
    }
...
exit:
	...
	wakeup_source_remove(&rf_data->cable_ws);
	__pm_relax(&rf_data->cable_ws);
	kfree(rf_data);
exit_nomem:
	the_rf_data = NULL;
