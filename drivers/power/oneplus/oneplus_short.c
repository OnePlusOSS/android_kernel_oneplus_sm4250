...
#define short_debug(fmt,...) \
	do { \
		if (short_d_on == true) { \
		printk(KERN_CRIT pr_fmt("=====[ONEPLUS_CHG]"fmt),##__VA_ARGS__); \
		} \
	} while (0)
...
static int __init oneplus_short_c_battery_status_init(char *str)
{
	sscanf(str, "%d", &short_c_battery_status);
	chg_err("short_c_battery_status[%d]\n", short_c_battery_status);
	return 0;
}
__setup("short_c_battery_status=", oneplus_short_c_battery_status_init);

static int __init oneplus_short_c_switch_status_init(char *str)
{
	sscanf(str, "%d", &short_c_switch_status);
	chg_err("short_c_switch_status[%d]\n", short_c_switch_status);
	return 0;
}
__setup("short_c_switch_status=", oneplus_short_c_switch_status_init);

static int __init oneplus_short_c_feature_sw_status_init(char *str)
{
	sscanf(str, "%d", &short_c_feature_sw_status);
	chg_err("short_c_feature_sw_status[%d]\n", short_c_feature_sw_status);
	return 0;
}
__setup("short_c_feature_sw_status=", oneplus_short_c_feature_sw_status_init);

static int __init oneplus_short_c_feature_hw_status_init(char *str)
{
	sscanf(str, "%d", &short_c_feature_hw_status);
	chg_err("short_c_feature_hw_status[%d]\n", short_c_feature_hw_status);
	return 0;
}
__setup("short_c_feature_hw_status=", oneplus_short_c_feature_hw_status_init);

int oneplus_short_c_batt_err_code_init(void)
{
	return short_c_battery_status;
}

int oneplus_short_c_batt_chg_switch_init(void)
{
	return short_c_switch_status;
}

int oneplus_short_c_batt_feature_sw_status_init(void)
{
	return short_c_feature_sw_status;
}

int oneplus_short_c_batt_feature_hw_status_init(void)
{
...
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
static bool oneplus_is_power_off_chg(struct oneplus_chg_chip *chip)
{
...
static bool oneplus_is_normal_boot(struct oneplus_chg_chip *chip)
{
...
//extern bool qpnp_is_power_off_charging(void)//debug for bring up 
...
static bool oneplus_is_normal_boot(struct oneplus_chg_chip *chip)
{
return true;//debug for bring up 
#if 0
...
static int oneplus_digit_num_len(const char *str)
{
	int len = 0;
...
static int oneplus_cmpint(const void *a, const void *b)
{
	return *(int *)a - *(int *)b;
}

bool oneplus_short_c_batt_is_prohibit_chg(struct oneplus_chg_chip *chip)
{
...
bool oneplus_short_c_batt_is_disable_rechg(struct oneplus_chg_chip *chip)
{
...
static void oneplus_short_c_batt_set_disable_rechg(struct oneplus_chg_chip *chip, bool disable)
{
	chip->short_c_batt.disable_rechg = disable;
}

bool oneplus_short_c_batt_get_cv_status(struct oneplus_chg_chip *chip)
{
	return chip->short_c_batt.cv_satus;
}

static bool oneplus_short_c_batt_data_is_mounted(void)
{
	struct file *fp;
...
static int oneplus_short_c_batt_read_file(struct oneplus_chg_chip *chip)
{
	struct file *fp;
	mm_segment_t old_fs;
	loff_t pos;
	...
	for (i = 0; i < ARRAY_SIZE(short_c_batt_items); i++) {
		...
		if (p != 0) {
			...
			strncpy(buf_tmp, p, oneplus_digit_num_len(p));
			buf_tmp[oneplus_digit_num_len(p)] = '\0';
	...
	chip->short_c_batt.cv_timer1
		= short_c_batt_items[2].value / ONEPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer2
		= short_c_batt_items[3].value / ONEPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer3
		= short_c_batt_items[4].value / ONEPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer4
		= short_c_batt_items[5].value / ONEPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer5
		= short_c_batt_items[6].value / ONEPLUS_CHG_UPDATE_INTERVAL_SEC;
...
static bool oneplus_is_algorithm_parameters_valid(struct oneplus_chg_chip *chip)
{
 ...
 */
static int oneplus_short_c_batt_write_err_code(int err_code,
...
static int oneplus_short_c_batt_write_exit_code(int exit_code,
...
static int oneplus_short_c_batt_write_chg_data(int current_ma, int volt_mv,
...
void oneplus_short_c_batt_update_change(struct oneplus_chg_chip *chip, int update_value)
{
	if (update_value == SHORT_C_BATT_UPDATE_PARM) {
		oneplus_short_c_batt_read_file(chip);
	} else if (update_value == SHORT_C_BATT_CLR_ERR_CODE) {
		oneplus_short_c_batt_write_err_code(0, 0, 0, 0, 0, 0);
...
static int oneplus_short_c_batt_get_average_vbatt_mv(struct oneplus_chg_chip *chip, bool reset)
{
	...
	if (valid_vbatt_count == ARRAY_SIZE(buf)) {
		avg_vbatt_mv = 0;
		sort(buf, ARRAY_SIZE(buf), sizeof(buf[0]), oneplus_cmpint, NULL);
...
void oneplus_chg_short_c_hw_check(struct oneplus_chg_chip *chip)
{
...
void oneplus_chg_short_c_ic_check(struct oneplus_chg_chip *chip)
{
	...
	chip->short_c_batt.ic_short_otp_st = oneplus_short_ic_otp_check();
	if (chip->short_c_batt.is_feature_hw_on
			== SHORT_C_BATT_FEATURE_HW_STATUS__OFF) {
		return;
	}
	if (chip->charger_exist && (short_ic_otp_st
		...
		oneplus_chg_turn_on_charging(chip);
		chg_debug("[Shortc_ic] oneplus_shorc_ic status changed! \
...
void oneplus_chg_short_c_battery_check(struct oneplus_chg_chip *chip)
{
	...
	oneplus_chg_short_c_hw_check(chip);
	oneplus_chg_short_c_ic_check(chip);
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oneplus_is_power_off_chg(chip) == true) {
			data_mounted_count++;
			if (data_mounted_count < 30
					&& oneplus_short_c_batt_data_is_mounted() == true) {//3min
				oneplus_short_c_batt_read_file(chip);
		...
		} else if (oneplus_is_normal_boot(chip) == false) {
			chg_err("oneplus_is_normal_boot[false]\n");
			return;
		}
	}
	...
	if (oneplus_is_algorithm_parameters_valid(chip) == false) {
	...
	if (chip->temperature <= LITTLE_COLD_TEMP_THRESHOLD || chip->temperature > WARM_TEMP_THRESHOLD) {//0~45
		...
		if (oneplus_short_c_batt_get_cv_status(chip) == true) {
			oneplus_short_c_batt_write_exit_code(EXIT_CODE__EX5_OUT_OF_TEMP_0_45,
	...
	if (chip->batt_full == true && chip->chging_on == false
		...
		oneplus_short_c_batt_write_chg_data(chip->icharging, chip->batt_volt,
		...
		if (full_over_10ma_count >= chip->short_c_batt.ex2_lower_ibatt_count) {
			oneplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_LOWER_IBATT,
		...
		if (full_over_20ma_count >= chip->short_c_batt.ex2_low_ibatt_count) {
			oneplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_LOW_IBATT,
				INVALID_CODE, INVALID_CODE, INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n",
		...
		if (full_over_50ma_count >= chip->short_c_batt.ex2_high_ibatt_count) {
			oneplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_HIGH_IBATT,
				INVALID_CODE, INVALID_CODE, INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n",
		...
		if (full_timer5_count <= chip->short_c_batt.full_timer5) {
			...
			oneplus_short_c_batt_set_disable_rechg(chip, true);
			oneplus_short_c_batt_get_average_vbatt_mv(chip, true);//reset
			return;
		}
		...
		if (full_timer5_count > chip->short_c_batt.full_timer5
			...
			full_vbatt_2 = oneplus_short_c_batt_get_average_vbatt_mv(chip, false);
			...
			if (full_vbatt_2 > 0 && chip->limits.iterm_ma != 0) {
				...
				if (exit_check_status == true) {
					oneplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_RBATT,
					...
					oneplus_short_c_batt_write_exit_code(EXIT_CODE__NOT_OVER_RBATT,
						chip->temperature, chip->temperature, full_vbatt_2, rbatt);
		...
		if (full_timer2_count > chip->short_c_batt.full_timer2
			...
			full_vbatt_n = oneplus_short_c_batt_get_average_vbatt_mv(chip, false);
		...
		if (full_timer3_count <= chip->short_c_batt.full_timer3) {
			...
			if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
				oneplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER3,
			...
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				if (full_timer3_count == chip->short_c_batt.full_timer3 + 1) {
					if ((chip->short_c_batt.batt_chging_cycles < chip->short_c_batt.batt_chging_cycle_threshold)
						...
						chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__FULL_ERR_CODE2;
						oneplus_short_c_batt_write_err_code(chip->short_c_batt.err_code, full_temp1,
				...
				if (full_timer3_count == chip->short_c_batt.full_timer3 + 2) {
					...
					if (chip->short_c_batt.err_code != SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4) {
						...
						oneplus_short_c_batt_write_exit_code(EXIT_CODE__TIMER3_OVERFLOW_INFO,
		...
		if (full_timer4_count <= chip->short_c_batt.full_timer4) {
			...
			oneplus_short_c_batt_get_average_vbatt_mv(chip, true);//reset
			if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
				oneplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER4,
					full_temp1, chip->temperature, INVALID_CODE, INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n", chip->temperature);
				short_debug("don't exit_check, T1[%d], T2[%d]\n", full_temp1, chip->temperature);
				goto cv_reset;
			...
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				...
				if (full_timer4_count == chip->short_c_batt.full_timer4 + 2) {
					...
					if (chip->short_c_batt.err_code != SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5) {
						...
						oneplus_short_c_batt_write_exit_code(EXIT_CODE__TIMER4_OVERFLOW,
							full_temp1, chip->temperature, full_vbatt1, full_vbatt_n);
					}
	...
	if (chip->batt_volt > chip->short_c_batt.short_c_bat_cv_mv && chip->icharging < 0
		...
		if (cv_pre_check_count == CV_PRE_CHECK_COUNT) {
			if (chip->short_c_batt.cv_satus == false) {
				//reset exit code
				oneplus_short_c_batt_write_exit_code(0, 0, 0, 0, 0);
				chip->short_c_batt.cv_satus = true;
	...
	if (chip->short_c_batt.cv_satus == true) {
		...
		if (cv_timer1_count == chip->short_c_batt.cv_timer1) {
			...
			oneplus_short_c_batt_write_err_code(SHORT_C_BATT_STATUS__CV_ERR_CODE1,
...
//full_reset:
	...
	oneplus_short_c_batt_set_disable_rechg(chip, false);
	oneplus_short_c_batt_get_average_vbatt_mv(chip, true);

	return;
}
#else /* CONFIG_ONEPLUS_SHORT_C_BATT_CHECK */
int oneplus_short_c_batt_err_code_init(void)
{
