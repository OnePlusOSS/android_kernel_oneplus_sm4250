...
int __attribute__((weak)) oneplus_get_fg_device_type(void)
{
	return 0;
}

void __attribute__((weak)) oneplus_set_fg_device_type(int device_type)
{
	return;
}
...
/* ONEPLUS 2013-08-24 wangjc Add begin for add adc interface. */
...
static struct oneplus_gauge_operations bq27541_gauge_ops = {
...
static void bq27541_hw_config(struct chip_bq27541 *chip)
{
	...
	oneplus_set_fg_device_type(chip->device_type);
...
static void register_gauge_devinfo(struct chip_bq27541 *chip)
{
	...
	//ret = register_device_proc("gauge", version, manufacture);//debug for bring up 
	if (ret) {
...
{
	int ui_soc = oneplus_chg_get_ui_soc();
...
bool oneplus_gauge_ic_chip_is_null(void)
{
	if (!gauge_ic) {
		return true;
	} else {
		...
		const struct i2c_device_id *id)
{
	...
	struct oneplus_gauge_chip	*chip;
	int rerun_num = 3;
	int rc =0 ;
...
static const struct of_device_id bq27541_match[] = {
	{ .compatible = "oneplus,bq27541-battery"},
	{ },
};
