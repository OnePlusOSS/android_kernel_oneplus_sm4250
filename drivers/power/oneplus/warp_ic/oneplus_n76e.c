...
static bool n76e_fw_check_frontline(struct oneplus_warp_chip *chip)
{
	...
	for (fw_line = 0; fw_line < (total_line - 1); fw_line++) {
		...
		data_buf[0] = 0;
		oneplus_warp_i2c_write(chip->client, 0x03, 1, &data_buf[0]);
...
static bool n76e_fw_check_lastline(struct oneplus_warp_chip *chip)
{
...
static int n76e_fw_write(struct oneplus_warp_chip *chip,
	const unsigned char *data_buf, unsigned int offset, unsigned int length)
{
	...
	while (count < (offset + length)) {
		...
		oneplus_warp_i2c_write(chip->client, 0x02, BYTES_TO_WRITE,
...
static int n76e_fw_update(struct oneplus_warp_chip *chip)
{
	unsigned char zero_buf[1] = {0};
	unsigned char addr_buf[2] = {0x00, 0x00};
	unsigned char temp_buf[1] = {0};
...
static int n76e_get_fw_verion_from_ic(struct oneplus_warp_chip *chip)
{
	unsigned char addr_buf[2] = {0x3B, 0xF0};
...
static void n76e_fw_check_then_recover(struct oneplus_warp_chip *chip)
{
	int update_result = 0;
...
struct oneplus_warp_operations oneplus_n76e_ops = {
...
#endif
#endif
	if(get_warp_mcu_type(chip) != ONEPLUS_WARP_MCU_HWID_N76E){
...
static const struct of_device_id n76e_match[] = {
	{ .compatible = "oneplus,n76e-fastcg"},
	{ },
};
...
MODULE_DESCRIPTION("Driver for oneplus warp n76e fast mcu");
