...
static int op10_set_fw_new_version(struct oneplus_warp_chip *chip);
static int op10_get_fw_old_version(struct oneplus_warp_chip *chip, u8 version_info[]);
...
static int check_flash_idle(struct oneplus_warp_chip *chip, u32 try_count)
{
	u8 rx_buf;
	int rc = 0;
	...
	do {
		rx_buf = 0xff;
		rc = oneplus_warp_i2c_read(chip->client, CMD_DEV_STATUS,1, &rx_buf);
		if (rc < 0) {
...
static int check_crc32_available(struct oneplus_warp_chip *chip, u32 try_count)
{
	u8 rx_buf;
	int rc = 0;
...
u32 crc32_sram(struct oneplus_warp_chip *chip)
{
...
static bool op10_fw_update_check(struct oneplus_warp_chip *chip)
{
	int i = 0;
	int ret = 0;
...
static int op10_fw_update(struct oneplus_warp_chip *chip)
{
	...
	while (fw_len) {
		...
		*((u32 *)rx_buf) = fw_offset;
		oneplus_warp_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
		msleep(1);
		...
		while (write_len_temp) {
			...
			oneplus_warp_i2c_write(chip->client, CMD_XFER_W_DAT, chunk_len, chip->firmware_data + fw_offset + chunk_index * I2C_RW_LEN_MAX);
			msleep(1);

			write_len_temp -= chunk_len;
			chunk_index++;
		}
		oneplus_warp_i2c_read(chip->client, CMD_PRG_START, 1, rx_buf);
		msleep(5);
	...
	*((u32 *)rx_buf) = chip->fw_data_count;
	oneplus_warp_i2c_write(chip->client, CMD_SET_CKSM_LEN, 4, rx_buf);
	msleep(5);
	...
	memset(rx_buf, 0, 4);
	oneplus_warp_i2c_read(chip->client, CMD_GET_CRC32, 4, rx_buf);
	...
	oneplus_warp_i2c_read(chip->client, CMD_USER_BOOT, 1, rx_buf);
...
static int op10_get_fw_old_version(struct oneplus_warp_chip *chip, u8 version_info[])
{
	...
	oneplus_warp_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
	msleep(1);
	oneplus_warp_i2c_read(chip->client, CMD_XFER_R_DATA, 4, rx_buf);
	if (*((u32 *)rx_buf) < fw_len_address) {
		...
		oneplus_warp_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
		msleep(1);
		oneplus_warp_i2c_read(chip->client, CMD_XFER_R_DATA, FW_VERSION_LEN, version_info);
	} else {
...
static int op10_set_fw_new_version(struct oneplus_warp_chip *chip)
{
	...
	if (*((u32 *)rx_buf) < fw_len_address - FW_VERSION_LEN) {
		oneplus_warp_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
		msleep(1);
		oneplus_warp_i2c_write(chip->client, CMD_XFER_W_DAT, FW_VERSION_LEN, version_info);
		msleep(1);
		oneplus_warp_i2c_read(chip->client, CMD_PRG_START, 1, rx_buf);
		msleep(10);
		if (check_flash_idle(chip, write_done_try_count) == -1) {
...
static int op10_get_fw_verion_from_ic(struct oneplus_warp_chip *chip)
{
...
static void op10_fw_check_then_recover(struct oneplus_warp_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
...
struct oneplus_warp_operations oneplus_op10_ops = {
...
#endif
#endif
	if (get_warp_mcu_type(chip) != ONEPLUS_WARP_ASIC_HWID_OP10) {
...
static const struct of_device_id op10_match[] = {
	{ .compatible = "oneplus,op10-fastcg"},
	{ },
};
...
MODULE_DESCRIPTION("Driver for oneplus warp op10 fast mcu");
