...
static bool stm8s_fw_check_frontline(struct oneplus_warp_chip *chip)
{
	...
	for (i = 0;i < READ_COUNT;i++) {	/*1508:448, 1503:192*/
		oneplus_warp_i2c_read(chip->client, 0x03, 16, &data_buf[0]);
		msleep(2);
		oneplus_warp_i2c_read(chip->client, 0x03, 16, &data_buf[16]);
		addr = 0x8800 + i * 32;
...
static bool stm8s_fw_check_lastline(struct oneplus_warp_chip *chip)
{
...
static int stm8s_fw_write(struct oneplus_warp_chip *chip,
		const unsigned char *data_buf, unsigned int offset, unsigned int length)
{
	...
	while (count < (offset + length)) {
		...
		oneplus_warp_i2c_write(chip->client, 0x02, BYTES_TO_WRITE, &data_buf[count+BYTE_OFFSET]);
		oneplus_warp_i2c_write(chip->client, 0x05, 1, &zero_buf[0]);
		oneplus_warp_i2c_read(chip->client, 0x05, 1, &temp_buf[0]);
		/*chg_err("lfc read 0x05, temp_buf[0]:0x%x\n", temp_buf[0]);*/

		/*write 16 bytes data to stm8s again*/
		oneplus_warp_i2c_write(chip->client, 0x02, BYTES_TO_WRITE, &data_buf[count+BYTE_OFFSET+BYTES_TO_WRITE]);
		oneplus_warp_i2c_write(chip->client, 0x05, 1, &zero_buf[0]);
		oneplus_warp_i2c_read(chip->client, 0x05, 1, &temp_buf[0]);
...
static int stm8s_fw_update(struct oneplus_warp_chip *chip)
{
	unsigned char zero_buf[1] = {0};
	unsigned char addr_buf[2] = {0x88, 0x00};
	unsigned char temp_buf[1] = {0};
...
update_fw:
	...
	for (i = 0; i < ERASE_COUNT; i++) {
		...
		oneplus_warp_i2c_write(chip->client, 0x04, 1, &zero_buf[0]);
		msleep(1);
		oneplus_warp_i2c_read(chip->client, 0x04, 1, &temp_buf[0]);
	...
	oneplus_warp_i2c_write(chip->client, 0x06, 1, &zero_buf[0]);
	oneplus_warp_i2c_read(chip->client, 0x06, 1, &temp_buf[0]);
...
static int stm8s_get_fw_verion_from_ic(struct oneplus_warp_chip *chip)
{
...
static void stm8s_fw_check_then_recover(struct oneplus_warp_chip *chip)
{
	int update_result = 0;
...
struct oneplus_warp_operations oneplus_stm8s_ops = {
...
static const struct of_device_id stm8s_match[] = {
	{ .compatible = "oneplus,stm8s-fastcg"},
	{ },
};
...
MODULE_DESCRIPTION("Driver for oneplus warp stm8s fast mcu");
