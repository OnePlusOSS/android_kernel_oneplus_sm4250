...
static struct oneplus_warp_chip *the_chip = NULL;
...
static int oneplus_i2c_dma_read(struct i2c_client *client, u16 addr, s32 len, u8 *rxbuf)
{
	int ret;
	s32 retry = 0;
	u8 buffer[2] = {0};
...
static int oneplus_i2c_dma_write(struct i2c_client *client, u16 addr, s32 len, u8 const *txbuf)
{
	int ret = 0;
	s32 retry = 0;
...
static int oneplus_warp_i2c_read(struct i2c_client *client, u8 addr, s32 len, u8 *rxbuf)
{
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
#if GTP_SUPPORT_I2C_DMA
...
static int oneplus_warp_i2c_write(struct i2c_client *client, u8 addr, s32 len, u8 const *txbuf)
{
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
#if GTP_SUPPORT_I2C_DMA
...
static bool rk826_fw_update_check(struct oneplus_warp_chip *chip)
{
	int ret = 0;
	u8 data_buf[4]= {0};
	...
	ret = oneplus_i2c_dma_write(chip->client, REG_MTP_SELECT, 4, (u8 *)(&mtp_select_flag));
	if (ret < 0) {
	...
	for (i = chip->fw_data_count - 11; i <= chip->fw_data_count - 4; i++) {
		...
		ret = oneplus_i2c_dma_write(chip->client, REG_MTP_ADDR, 4, (u8 *)(&mtp_addr_flag));
		if (ret < 0) {
		...
		do {
			ret  = oneplus_i2c_dma_read(chip->client, REG_MTP_SELECT, 4, data_buf);
			if (ret < 0) {
		...
		ret = oneplus_i2c_dma_read(chip->client, REG_MTP_DATA, 4, data_buf);
		if (ret < 0) {
...
int WriteSram(struct oneplus_warp_chip *chip, const u8 *buf, u32 size)
{
	u8 offset = 0;
	u16 reg_addr;
	int ret = 0;
	...
	while (size) {
		...
		for (i = 0; i < cur_size / 4; i++) {
			...
			ret = oneplus_i2c_dma_write(chip->client, reg_addr, 4, tx_buff);
			if (ret < 0) {
				chg_err("write SRAM fail");
				return -1;
			}
		}
		//ret = oneplus_i2c_dma_read(chip->client, REG_STATE, 4, readbuf);
		//chg_err("teh REG_STATE1: %d", *(u32*)readbuf);
		ret = oneplus_i2c_dma_write(chip->client, REG_HOST, 4, (u8 *)(&rec_0O_flag));
		//mdelay(3);
		...
		do {
			...
			//msleep(10);
			ret = oneplus_i2c_dma_read(chip->client, REG_SLAVE, 4, readbuf);
			if (ret < 0) {
		...
		ret = oneplus_i2c_dma_write(chip->client, REG_HOST, 4, (u8 *)(&rec_01_flag));
		//write rec_00 into host
		if (ret < 0) {
			chg_err("write rec_00 into host");
			return -1;
...
int DownloadFirmware(struct oneplus_warp_chip *chip, const u8 *buf, u32 size)
{
...
static int rk826_fw_update(struct oneplus_warp_chip *chip)
{
	int ret = 0;
	int iTryCount = 3;
	struct_req req = {0};
	...
	oneplus_i2c_dma_write(chip->client, REG_SYS0, 4, (u8 *)(&force_update_flag));
	oneplus_i2c_dma_write(chip->client, REG_RESET, 4, (u8 *)(&sw_reset_flag));
	while (iTryCount) {
		ret = oneplus_i2c_dma_write(chip->client, REG_HOST, 4, (u8 *)(&sync_flag));
		if (ret < 0) {
		...
		//2.check ~sync
		ret = oneplus_i2c_dma_read(chip->client, REG_HOST, 4, read_buf);
	...
	// read reg_state
	ret = oneplus_i2c_dma_read(chip->client, REG_STATE, 4, read_buf);
	if (ret<0) {
		chg_err("write rec_01 flag failed!");
		goto update_fw_err;
	}
...
static int rk826_get_fw_verion_from_ic(struct oneplus_warp_chip *chip)
{
	...
	if (oneplus_is_power_off_charging(chip) == true || oneplus_is_charger_reboot(chip) == true) {
		...
		//first:set address
		rc = oneplus_warp_i2c_write(chip->client, 0x01, 2, &addr_buf[0]);
		if (rc < 0) {
			chg_err(" i2c_write 0x01 error\n" );
			return FW_CHECK_FAIL;
		}
		msleep(2);
		oneplus_warp_i2c_read(chip->client, 0x03, 4, data_buf);
...
static void rk826_fw_check_then_recover(struct oneplus_warp_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
...
struct oneplus_warp_operations oneplus_rk826_ops = {
	.fw_update = rk826_fw_update,
	.fw_check_then_recover = rk826_fw_check_then_recover,
	.set_switch_mode = opchg_set_switch_mode,
	.eint_regist = oneplus_warp_eint_register,
	.eint_unregist = oneplus_warp_eint_unregister,
	...
	.is_power_off_charging = oneplus_is_power_off_charging,
	.get_reset_gpio_val = oneplus_warp_get_reset_gpio_val,
	.get_switch_gpio_val = oneplus_warp_get_switch_gpio_val,
	.get_ap_clk_gpio_val = oneplus_warp_get_ap_clk_gpio_val,
...
static int rk826_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
#endif
	if (get_warp_mcu_type(chip) != ONEPLUS_WARP_ASIC_HWID_RK826) {
	...
	chip->vops = &oneplus_rk826_ops;
	chip->fw_mcu_version = 0;

	oneplus_warp_gpio_dt_init(chip);
	opchg_set_clock_sleep(chip);
	oneplus_warp_delay_reset_mcu_init(chip);
	...
	if (chip->warp_fw_update_newmethod) {
		if (oneplus_is_rf_ftm_mode()) {
			oneplus_warp_fw_update_work_init(chip);
		}
	} else {
		oneplus_warp_fw_update_work_init(chip);
	}

	oneplus_warp_init(chip);
...
static const struct of_device_id rk826_match[] = {
	{ .compatible = "oneplus,rk826-fastcg"},
	{ },
};
...
MODULE_DESCRIPTION("Driver for oneplus warp rk826 fast mcu");
