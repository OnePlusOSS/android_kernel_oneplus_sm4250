...
bool oneplus_warp_adapter_update_is_tx_gpio(unsigned long gpio_num)
{
	if (!the_chip) {
		return false;
	}
...
bool oneplus_warp_adapter_update_is_rx_gpio(unsigned long gpio_num)
{
	if (!the_chip) {
		return false;
	}
...
struct oneplus_adapter_operations oneplus_adapter_ops = {
	.adapter_update = warp_adapter_update_handle,
};

static int __init adapter_ic_init(void)
{
	struct oneplus_adapter_chip *chip = NULL;
	...
	the_chip = adapter_ic;
	chip = kzalloc(sizeof(struct oneplus_adapter_chip), GFP_KERNEL);
	if (!chip) {
...
//       chip->client = client;
//       chip->dev = &client->dev;
	chip->vops = &oneplus_adapter_ops;
	oneplus_adapter_init(chip);
...
MODULE_DESCRIPTION("Driver for oneplus adapter ic stm8s");
