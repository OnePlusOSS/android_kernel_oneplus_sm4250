...
#include "qg-defs.h"
#include <linux/rtc.h>
#include "../../oneplus/oneplus_gauge.h"
...
static int oneplus_asic_vadc_map_voltage_temp(const struct asic_vadc_map_pt *pts,
...
int qg_get_battery_temp(struct qpnp_qg *chip, int *temp)
{
	...
	*temp = result;
	pr_debug("v_temp = %d, ichg = %d, temp_before = %d, temp_after = %d\n",
...
int qg_get_battery_current(struct qpnp_qg *chip, int *ibat_ua)
{
	...
	*ibat_ua = qg_iraw_to_ua(chip, last_ibat);
	...
	g_oneplus_qg_ibta = *ibat_ua / 1000;
...
static int op_get_batt_remain_capacity(struct qpnp_qg *chip)
{
	int64_t full_capacity;
	int remain_soc;

	qg_get_learned_capacity(chip, &full_capacity);
	qg_get_battery_capacity(qpnp_gauge_ic, &remain_soc);
	return (remain_soc * full_capacity)/100;
}
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_REMAINING_CAPACITY:
		pval->intval = op_get_batt_remain_capacity(chip);
		break;
...
static int oneplus_get_battery_voltage(void)
{
	int voltage = 0;
	int rc = 0;
	rc = qg_get_battery_voltage(qpnp_gauge_ic, &voltage);
	pr_debug("oneplus_get_battery_voltage = %d, rc= %d\n",voltage,rc);
	return voltage/1000;
}

#define TEMP_AVERAGE_SIZE 5
static int oneplus_get_battery_temperature(void)
{
	...
	pr_debug("oneplus_get_battery_temperature avg_temp = %d, curr_temp = %d\n",avg_temp,curr_temp);
	return avg_temp;
}

static int oneplus_get_battery_current(void)
{
	int bat_current = 0;
	int rc = 0;
	qg_get_battery_current(qpnp_gauge_ic, &bat_current);
	pr_debug("oneplus_get_battery_current = %d, rc= %d\n",bat_current,rc);
	return bat_current/1000;
}

static int oneplus_get_battery_capacity(void)
{
	int capacity = 0;
	int rc = 0;
	rc = qg_get_battery_capacity(qpnp_gauge_ic, &capacity);
	pr_debug("oneplus_get_battery_capacity = %d, rc= %d\n",capacity,rc);
	return capacity;
}

static int oneplus_get_battery_cc(void)
{
	int batt_cc = 3500;
	return batt_cc;
}

static int oneplus_get_battery_fcc(void)
{
	int batt_fcc = 5000;//full charge capacity
	return batt_fcc;
}

static int oneplus_get_battery_remain_capacity(void)
{
	int remain_capacity = 0;
	int rc = 0;
	rc = qg_get_battery_capacity(qpnp_gauge_ic, &remain_capacity);
	remain_capacity = 5000 * remain_capacity / 100;
	pr_debug("oneplus_get_battery_remain_capacity = %d, rc= %d\n",remain_capacity,rc);
	return remain_capacity;
}

static int oneplus_get_battery_soh(void)
{
	int soh = 100;
	return soh;
}

static void oneplus_set_battery_full(bool full)
{
	/* Do nothing */
}

static int oneplus_update_battery_dod0(void)
{
	return 0;
}

static bool oneplus_get_battery_authenticate(void)
{
	bool rc = 0;
...
static int oneplus_update_soc_smooth_parameter(void)
{
	return 0;
}

static struct oneplus_gauge_operations qonp_gauge_ops = {
	.get_battery_mvolts = oneplus_get_battery_voltage,
	.get_battery_temperature = oneplus_get_battery_temperature,
	.get_batt_remaining_capacity = oneplus_get_battery_remain_capacity,
	.get_battery_soc = oneplus_get_battery_capacity,
	.get_average_current = oneplus_get_battery_current,
	.get_battery_fcc = oneplus_get_battery_fcc,
	.get_battery_cc = oneplus_get_battery_cc,
	.get_battery_soh = oneplus_get_battery_soh,
	.get_battery_authenticate = oneplus_get_battery_authenticate,
	.set_battery_full = oneplus_set_battery_full,
	.get_battery_mvolts_2cell_max = oneplus_get_battery_voltage,
	.get_battery_mvolts_2cell_min = oneplus_get_battery_voltage,
	.update_battery_dod0 = oneplus_update_battery_dod0,
	.update_soc_smooth_parameter = oneplus_update_soc_smooth_parameter,
};
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	struct qpnp_qg *chip;
	struct oneplus_gauge_chip	*oneplus_chip;
	...
	if (chip->enable_qpnp_qg) {
		oneplus_chip = devm_kzalloc(&pdev->dev,
			sizeof(struct oneplus_gauge_chip), GFP_KERNEL);
		if (!oneplus_chip) {
			qpnp_gauge_ic = NULL;
			pr_err("oneplus_chip kzalloc failed.\n");
			return -ENOMEM;
		}
		oneplus_chip->dev = &pdev->dev;
		oneplus_chip->gauge_ops = &qonp_gauge_ops;
		oneplus_gauge_init(oneplus_chip);
	} else {
		pr_info("qpnp_qg_probe oneplus_gauge is not enable\n");
	}
